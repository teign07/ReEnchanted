...
**September 22 decision:** the Digital Standing Order is retired. With
`MONTHLY_ISSUES_OPEN_TO_ALL=true`, every installation may obtain a short-lived,
installation-bound monthly session without an Apple or Bound Year purchase.
The signed manifest, private R2 files, and installation binding still apply.
The paid-proof steps below remain implemented for a possible future paid offer;
they are bypassed while the open-shelf flag is set. The app and Worker switches
must change together.

## Request flow

1. The app obtains the existing installation-bound print-desk session.
2. `POST /monthly-issues/session` exchanges that session plus `{ signedTransactions: [JWS], membershipID?: string }` for a token valid for at most ten minutes. With the open shelf enabled, the app sends no purchase proof; the paid path caps the token by paid-through expiry. Proofs are bounded to two signed transactions and 64 KiB total request bytes.
...
- Worker private R2 binding `MONTHLY_ISSUE_FILES` → bucket `reenchanted-monthly-issues` (created 23 September 2026, r2.dev access disabled). Store `manifest.envelope.json` and `assets/ASSET_ID`. Never enable public R2 access.
- `MONTHLY_APPLE_ENVIRONMENT`: `Sandbox` or `Production` when the paid digital gate is offered again.
- If the paid gate returns, Apple credentials: `MONTHLY_APPLE_PRIVATE_KEY` (App Store Server API P8 text), `MONTHLY_APPLE_KEY_ID`, `MONTHLY_APPLE_ISSUER_ID`, and `MONTHLY_APPLE_APP_ID` (required in Production).
- If the paid gate returns, `MONTHLY_APPLE_ROOT_CERTIFICATES`: JSON array of base64 DER Apple root certificates obtained from Apple's certificate authority. These are public trust anchors, not credentials. Never accept trust anchors from a request.
...
**Configured 23 September 2026.** Publisher key ID `reenchanted-publisher-2026-09`; public key `qlO/TluW8kH+biDJa+eJoWu6BY7a6WKduKF3LNhCBw4=` is in both the app's Info.plist and the Worker's `MONTHLY_ISSUE_MANIFEST_PUBLIC_KEY`. The private key is `~/.reenchanted-publishing/monthly-issue-private.key` on the publishing Mac (0600, outside the repo); back it up somewhere private, because losing it means rotating the key through an app release. An empty signed shelf (`issues: []`) is published so the endpoint answers 200 before the first issue. Publish a prepared, signed release with `scripts/publish_monthly_release.sh RELEASE_DIR` (dry run) then `--publish`: it re-verifies the signature and every asset digest, uploads assets first and the envelope last, and reads the live manifest back. Absent configuration still fails closed. Worker Node compatibility is enabled for Apple's verifier. See [Apple's library](https://github.com/apple/app-store-server-library-node) and [Cloudflare's Node compatibility reference](https://developers.cloudflare.com/workers/runtime-apis/nodejs/).
