...
const env = () => ({GMAIL_RECOVERY_DELIVERY_ENABLED:'true',GMAIL_CLIENT_ID:'fixture-client',GMAIL_CLIENT_SECRET:'fixture-secret',GMAIL_REFRESH_TOKEN:'fixture-refresh'});
...
test('fixed sender, reply address, encoded body and input reject header injection', () => {
  const mime = Buffer.from(recoveryMessage(message),'base64url').toString();
  assert.ok(mime.includes('From: ReEnchanted <snow.potions@gmail.com>\r\n'));
  ...
  for(const change of [{recipient:'reader@example.invalid\r\nBcc: attacker@example.invalid'},{recipient:'a@example.invalid,b@example.invalid'},{membershipID:'sub_bad\n'},{secret:'invalid'}])
    assert.throws(()=>recoveryMessage({...message,...change}), /invalid_recovery_message/);
});
test('disabled or unconfigured delivery performs no network calls', async () => {
  for(const e of [{}, {...env(),GMAIL_REFRESH_TOKEN:''}]) {
