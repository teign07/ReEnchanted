...
enum ThreadOfTheMonth {
    ...
    struct Progress: Equatable {
        ...
        var litDays: Int
        /// Pages already present in those days. This is evidence for a warm
        /// retrospective sentence, not a denominator or target.
        var keptPages: Int
    }
    ...
    static func progress(forMonthContaining anchor: Date, days: [BookDay], calendar: Calendar) -> Progress {
        ...
        let monthDays = days.filter { day in
            let start = BookDay.startDate(for: day.id, fallback: day.date, calendar: calendar)
        ...
        return Progress(
            ...
            litDays: litDayCount(inMonthContaining: anchor, days: monthDays, calendar: calendar),
            keptPages: monthDays.reduce(0) { $0 + $1.capturedPages.count }
        )
    }
}
