...
struct BookOfYouShareArtifact: Equatable {
    ...
    var seed: Int
    /// Overprinted across the top of the Page, above its title. The loose Page
    /// that falls out of the Book at the very start of everything lands with
    /// its warning already on it, so the warning belongs to the Page, not to
    /// the app's narration. Stays put when the reader writes their own first
    /// line and the rest of the Page re-forms around it.
    ...
    var body: some View {
        ...
        ZStack {
            ...
            VStack(spacing: 24) {
                ...
                if let stamp = artifact.stamp?.nonEmpty {
                    // Overprinted, so it outweighs the Page's own title: this
                    // is the thing the Page came to say. Fixed height keeps the
                    // rest of the plate from shifting when the copy changes.
                    Text(stamp)
                        .font(.system(size: 62, weight: .heavy, design: .serif))
                        .foregroundStyle(BookPalette.rubricRed)
                        .multilineTextAlignment(.center)
                        .lineSpacing(1)
                        .lineLimit(3)
                        .minimumScaleFactor(0.55)
                        .frame(width: Self.contentWidth, height: 196)
                        .shadow(color: BookPalette.page.opacity(0.8), radius: 1, x: 0, y: 1)
                }
                ...
                Text(artifact.title)
                    ...
                    .frame(width: Self.contentWidth, height: artifact.stamp?.nonEmpty == nil ? 128 : 78)
...
struct BookOfYouPressedPageSpotlight: View {
    ...
    var body: some View {
        ...
        .accessibilityLabel("Pressed Book of You share page. \(artifact.stamp.map { $0 + ". " } ?? "")Titled \(artifact.title). \(artifact.excerpt)")
    }
...
struct SurfaceCard: View {
    ...
    private var bookInterjectionPhysicalAct: String? {
        surface.payload.metadata["bookInterjectionPhysicalAct"]?.nonEmpty
    }

    private var bookInterjectionTargetExcerpt: String? {
        surface.payload.metadata["bookInterjectionTargetExcerpt"]?.nonEmpty
    }

    private var bookCharacterCadence: String? {
        surface.payload.metadata["bookCharacterCadence"]?.nonEmpty
    }

    private var bookCharacterPromptStyle: Font.TextStyle {
        switch bookCharacterCadence {
        case "small-hand": return .title3
        case "spilling-over": return .title
        default: return .title2
        }
    }

    private var bookCharacterCardTilt: Double {
        switch bookCharacterCadence {
        case "crooked": return surface.id.stableHash.isMultiple(of: 2) ? -1.15 : 1.05
        case "both-paws": return 0
        default: return surface.id.stableHash.isMultiple(of: 2) ? -0.6 : 0.5
        }
    }

    private var bookInterjectionSymbol: String {
        switch bookInterjectionPhysicalAct {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text(surface.prompt)
                .font(.system(bookCharacterPromptStyle, design: .serif, weight: .semibold))
                .foregroundStyle(BookPalette.ink)
                .fixedSize(horizontal: false, vertical: true)
                .padding(.trailing, 26)
            ...
            if let margin = bookActedMargin {
                VStack(alignment: .leading, spacing: 5) {
                    Label(margin.title.uppercased(), systemImage: bookInterjectionSymbol)
                        .font(.caption2.weight(.black))
                        .tracking(0.9)
                    if bookInterjectionPhysicalAct == "underline",
                       let excerpt = bookInterjectionTargetExcerpt {
                        Text("“\(excerpt)”")
                            .font(.system(.callout, design: .serif, weight: .semibold))
                            .underline(true, color: visualStyle.accent.opacity(0.9))
                            .accessibilityLabel("The Book underlined: \(excerpt)")
                    }
                    Text(margin.line)
                        .font(.system(
                            bookInterjectionPhysicalAct == "sprawling-hand" ? .body : .callout,
                            design: .serif
                        ).italic())
                        .lineSpacing(2)
                }
        ...
        .rotationEffect(.degrees(bookCharacterCardTilt))
    }
...
struct SwipeDismissSurfaceCard: View {
    ...
    @State private var sealEngaged = false
    ...
    /// True exactly when letting go now would let the page pass. The seal and
    /// the tick both key off this rather than a nearby-looking fraction, so the
    /// Book never shows a sealed page it would then hand back.
    private var wouldDismissOnRelease: Bool {
        dragProgress >= 1
    }
    ...
    /// closes as the drag crosses the dismiss threshold.
    private var marginReveal: some View {
        BookMarginReveal(
            label: "Let it pass",
            dragProgress: dragProgress,
            sealed: wouldDismissOnRelease || isTucking
        )
    }
    ...
    /// Fires once on each crossing of the commit threshold, in either
    /// direction, never per frame, or the drag would buzz continuously.
    private func updateSealEngagement() {
        let engaged = wouldDismissOnRelease
        guard engaged != sealEngaged else { return }
        sealEngaged = engaged
        BookFeedback.sealTick(engaged: engaged)
    }
...
private struct SurfaceRetirementPlaceholder: View {
    ...
    var body: some View {
        ...
        .accessibilityElement(children: .ignore)
        .accessibilityLabel("I'm choosing another page")
    }
}
...
struct SwipeDismissBookOfYouHero<Content: View>: View {
    ...
    /// True exactly when letting go now would let the page pass.
    private var wouldDismissOnRelease: Bool {
        dragProgress >= 1
    }
...
/// The oxblood margin a page slips into as it is swiped away, with a wax seal
/// that closes at the moment letting go would actually part with the page.
///
/// Shared by every swipe-dismiss surface, so releasing a full card and
/// releasing a single kept scrap are the same gesture in the reader's hand.
/// Three hand-copied versions of this had already drifted apart once.
private struct BookMarginReveal: View {
    let label: String
    let dragProgress: CGFloat
    let sealed: Bool
    /// Cards stand a full 176pt; a scrap row is a smaller object, so the
    /// margin it slips into is proportionate to what is being pushed.
    var minHeight: CGFloat = 176
            ...
            ZStack {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        if !canBraid {
                            Text("I need one true fragment kept today before I can braid.")
                                .font(.system(.footnote, design: .serif))
...
struct FragmentRow: View {
    ...
    /// Shorter than a full surface card's 96pt, because a row is a smaller
    /// object under the thumb: the travel stays proportionate to what is
    /// being pushed, even though the feel of committing is the same.
    ...
    /// True exactly when letting go now would remove the scrap.
    private var wouldDismissOnRelease: Bool {
        dragProgress >= 1
    }
...
struct ReturnedStacksDailyHeader: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text("One rhyme. One long memory. One page the Stacks chose purely to stop me getting predictable.")
...
struct BookOfYouCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            // Below the ritual ending, never above it: the Book gets the last
            // word on the day, and then asks one thing in a quieter voice.
            if let question = details.backwardQuestion {
                Divider()
                    .overlay(visualStyle.accent.opacity(0.28))
                    .padding(.top, 4)
                Text(question)
                    .font(.system(.footnote, design: .serif))
                    .italic()
...
struct BookPresenceSilhouette: View {
    ...
    private var mark: BookMaterialMark {
        .current(in: interior, greyIsInsideCover: greyIsInsideCover)
    }
    ...
    private var accessibilityDescription: String {
        ...
        case .mischievous: disposition = "sitting crooked and hiding something"
        ...
        let evidence: String
        switch mark {
        case .greyScar: evidence = " One edge has gone pale where familiarity entered the Book."
        ...
        case .mischief: evidence = " A crooked pencil mark has escaped across the cover."
        case .longGame: evidence = " Its ribbon marks a longer piece of business."
        case .unmarked: evidence = ""
        }
        return "I'm \(disposition).\(evidence)"
    }
    ...
    private var materialEvidence: some View {
        ...
        case .greyScar:
            ZStack {
                Rectangle()
                    .fill(BookPalette.paper.opacity(0.82))
        ...
        case .mischief:
            Image(systemName: "scribble.variable")
                ...
                .foregroundStyle(BookPalette.paper.opacity(0.86))
...
/// A literal folded corner, large enough to read as something the Book did
/// rather than as a bookmark icon describing the idea of a dog-ear.
struct BookInterjectionDogEarMark: View {
    var tint: Color
    ...
    var body: some View {
        ZStack(alignment: .topTrailing) {
            BookDogEar()
                .fill(BookPalette.paper)
                .shadow(color: BookPalette.ink.opacity(0.28), radius: 2, x: -2, y: 2)
            BookDogEar()
            ...
            .stroke(tint.opacity(0.46), style: StrokeStyle(lineWidth: 1, dash: [3, 2]))
        }
        .allowsHitTesting(false)
    }
}

/// Ink transferred by a paw or thumb. It intentionally has no label: the mark
/// is the interjection.
struct BookInterjectionSmudgeMark: View {
    var tint: Color
    ...
    var body: some View {
        ZStack {
            ForEach(0..<7, id: \.self) { index in
                Capsule()
                    .fill(tint.opacity(0.055 + Double(index % 3) * 0.025))
                    .frame(width: CGFloat(18 + index * 3), height: CGFloat(5 + index % 2))
                    .rotationEffect(.degrees(Double(index * 11 - 28)))
                    .offset(
                        x: CGFloat((index % 3) * 7 - 7),
                        y: CGFloat(index * 8 - 24)
                    )
            }
        }
        .blur(radius: 0.55)
...
/// Two leaves refusing to close flush: a returned Page made physical.
struct BookInterjectionOpenLeafMark: View {
    var tint: Color
    ...
    var body: some View {
        HStack(spacing: -5) {
            RoundedRectangle(cornerRadius: 2, style: .continuous)
                .fill(BookPalette.paper)
                .overlay { RoundedRectangle(cornerRadius: 2).stroke(tint.opacity(0.52), lineWidth: 0.8) }
                .rotationEffect(.degrees(-7), anchor: .bottomTrailing)
            RoundedRectangle(cornerRadius: 2, style: .continuous)
                .fill(BookPalette.paper)
                .overlay { RoundedRectangle(cornerRadius: 2).stroke(tint.opacity(0.52), lineWidth: 0.8) }
                .rotationEffect(.degrees(7), anchor: .bottomLeading)
        }
        .shadow(color: BookPalette.ink.opacity(0.22), radius: 2, y: 1)
    }
}
...
enum BookPalette {
    ...
    /// The red the plate renderers already use for corner marks and closing
    /// lines. Reserved for things overprinted on a page rather than written
    /// into it: a warning, a stamp, a correction.
    static let rubricRed = Color(red: 0.54, green: 0.12, blue: 0.13)
}
...
struct PageVisualStyle {
    ...
    static func style(for type: BookPageType) -> PageVisualStyle {
        ...
        case .bookAside:
            return PageVisualStyle(
                accent: Color(red: 0.55, green: 0.24, blue: 0.27),
                symbolColor: Color(red: 0.55, green: 0.24, blue: 0.27),
                paperTop: Color(red: 0.95, green: 0.86, blue: 0.72),
                paperMiddle: Color(red: 0.84, green: 0.69, blue: 0.57),
                paperBottom: Color(red: 0.57, green: 0.40, blue: 0.40),
                scrapColor: Color(red: 0.87, green: 0.73, blue: 0.61),
                sideMarginalia: "IlluminationScrapS03_21",
                cornerMarginalia: "IlluminationScrapS01_16",
                smallMarginalia: "IlluminationScrapS03_08",
                watermarkMarginalia: "MarginaliaSeal",
                sideMarginaliaWidth: 70,
                cornerMarginaliaWidth: 84,
                sideMarginaliaOpacity: 0.44,
                cornerMarginaliaOpacity: 0.38,
                watermarkOpacity: 0.13,
                scrapWidth: 92,
                scrapHeight: 34
            )
...
struct OnboardingFlowView: View {
    ...
    struct Result {
        ...
        var pageChangeNoticed: String
        var rutStrongest: String
        var routineMemory: String
        var mostAlive: String
        var awakeMemory: String
        var magicSource: String
        var roleHands: String
        /// Role id the Book named and the reader accepted.
        var roleID: String
        /// Role id the Book named first, if the reader said "that's not me".
        var refusedRoleID: String
        var snack: String
    ...
    @State private var didLivingInkChange = false
    @State private var livingInkChangeTask: Task<Void, Never>?
    ...
    @State private var onboardingPhotoMessage = "Your handmade PDF is already waiting. You can also place one of your photos in it."
    ...
    @AppStorage("onboardingDraftPageChangeNoticed") private var pageChangeNoticed = ""
    @AppStorage("onboardingDraftRutStrongest") private var rutStrongest = ""
    @AppStorage("onboardingDraftRoutineMemory") private var routineMemory = ""
    /// Which ordinary hour the Book picks apart: drove / cooked / walked.
    @AppStorage("onboardingDraftRecallScenario") private var recallScenario = ""
    /// One character per asked question: "1" kept, "0" gone, "" unanswered.
    /// Persisted so backing out mid-interrogation resumes where they were.
    @AppStorage("onboardingDraftRecallAnswers") private var recallAnswersRaw = ""
    @AppStorage("onboardingDraftMostAlive") private var mostAlive = ""
    @AppStorage("onboardingDraftAwakeMemory") private var awakeMemory = ""
    @AppStorage("onboardingDraftMagicSource") private var magicSource = ""
    // The third axis of the reader's role: what the hands do with something
    // good once it has been found. Asked outright because it is the one that
    // changes what the Book should hand over next.
    @AppStorage("onboardingDraftHands") private var roleHands = ""
    // The role the Book named, and, if the reader tapped "that's not me": the
    // one it named first. A refused role is kept: being wrong about somebody in
    // a specific way is worth more later than pretending it never happened.
    @AppStorage("onboardingDraftRoleID") private var chosenRoleID = ""
    @AppStorage("onboardingDraftRoleRefused") private var refusedRoleID = ""
    ...
    @State private var isOnboardingPagewrightPresented = false
    @AppStorage("onboardingDraftPagewrightImagePath") private var onboardingPagewrightImagePath = ""
    @AppStorage("onboardingDraftPagewrightTitle") private var onboardingPagewrightTitle = ""
    @AppStorage("onboardingDraftPagewrightTemplate") private var onboardingPagewrightTemplate = ""
    @AppStorage("onboardingDraftPagewrightSourceSignature") private var onboardingPagewrightSourceSignature = ""
    @State private var onboardingPagewrightError = ""
    ...
    static let openingMiniStepCount = 9
    private let stepCount = 15
    private let currentOnboardingFlowVersion = 15
    ...
    /// Computed rather than fixed so the first answer can name the word that
    /// actually went. The Book is not scoring anybody: a reader who claims it
    /// and sees they were wrong has learned the same thing either way, and one
    /// who really caught it gets the small thrill of being right.
    private var pageChangeChoices: [OnboardingChoice] {
        [
            OnboardingChoice(
                id: "caught",
                title: "I caught it",
                detail: "It was \u{201C}\(livingInkProofParts.word.trimmingCharacters(in: .punctuationCharacters))\u{201D}. Was that yours?",
                symbol: "eye.fill"
            ),
            OnboardingChoice(id: "uncertain", title: "Something shifted", detail: "I saw movement, but I couldn't say what went missing.", symbol: "eye.trianglebadge.exclamationmark"),
            OnboardingChoice(id: "missed", title: "I missed it", detail: "The Page changed before I knew there was anything to catch.", symbol: "eye.slash")
        ]
    }
    private let routineMemoryChoices = [
        OnboardingChoice(id: "full", title: "Most of it", detail: "I can replay the ordinary minutes, not only the ending.", symbol: "film.stack"),
        OnboardingChoice(id: "islands", title: "A few sharp pieces", detail: "There are scenes, with blank water between them.", symbol: "circle.grid.cross"),
        OnboardingChoice(id: "result", title: "Mostly the result", detail: "I remember arriving or finishing, not much of the passage.", symbol: "flag.checkered"),
        OnboardingChoice(id: "blank", title: "Almost nothing", detail: "It happened, but it barely left a trace.", symbol: "rectangle.dashed")
    ]
    ...
    /// Ordered to match `ReaderRoleRegistry.hands`.
    private let handsChoices = [
        OnboardingChoice(id: "keeping", title: "I keep it", detail: "Somewhere safe, where I can find it again.", symbol: "archivebox"),
        OnboardingChoice(id: "telling", title: "I tell somebody", detail: "It isn't quite real until I have.", symbol: "bubble.left.and.bubble.right"),
        OnboardingChoice(id: "making", title: "I make something of it", detail: "It turns into a thing that didn't exist before.", symbol: "hammer"),
        OnboardingChoice(id: "returning", title: "I go back for more", detail: "If there was one, there are others.", symbol: "arrow.triangle.2.circlepath"),
        OnboardingChoice(id: "still", title: "I sit with it", detail: "I don't do anything. I let it stay a while.", symbol: "leaf")
    ]
    ...
    private let whisperChoices = [
        ...
        OnboardingChoice(id: "inside", title: "Only the necessary knocks", detail: "No ordinary whispers or keepable prompts. Only the Book's brief attention knocks arrive outside the covers.", symbol: "bell.slash")
    ]
    ...
    var body: some View {
        ...
        #endif
        .fullScreenCover(isPresented: $isOnboardingPagewrightPresented) {
            PagewrightSheet(
                keptPages: onboardingPagewrightScraps,
                bookwideAchievementContext: onboardingPagewrightAchievementContext,
                initialPageIDs: onboardingPagewrightScraps.map(\.id),
                initialPDFURL: nil,
                initialPNGURL: nil,
                onExportPDF: { _ in nil },
                onExportPNG: { draft in renderOnboardingPagewright(draft) },
        ...
        .onAppear {
            ...
            if openingMiniStep > Self.openingMiniStepCount - 1 {
                openingMiniStep = Self.openingMiniStepCount - 1
            }
            if step >= stepCount {
                step = stepCount - 1
            }
            ...
            scheduleLivingInkRutProofIfNeeded()
            guard !reduceMotion else { return }
        ...
        .onDisappear {
            ...
            livingInkChangeTask?.cancel()
            livingInkChangeTask = nil
        }
    }
    ...
    private func onboardingPageScroll(
        ...
        ScrollViewReader { scrollProxy in
            ...
            .onChange(of: openingMiniStep) { _, _ in
                ...
                scheduleOpeningOvertureTitleFadeIfNeeded()
                scheduleLivingInkRutProofIfNeeded()
                // House rule: moving forward always lands the reader at the
                // top of the next beat. Anything else means a beat can open
                // halfway down itself, which is how the opening came to feel
                // abrupt: the reader arrives mid-thought and has to scroll
                // back to find out what they are being asked.
                turnOnboardingPage(to: "onboarding-page-top", with: scrollProxy)
            }
            .onChange(of: mostAlive) { _, _ in
                awakeMemory = ""
            }
    ...
    private var onboardingActIndex: Int {
        switch step {
        case 0: return 0
        case 1...9: return 1
        case 10...13: return 2
    ...
    private var onboardingActTitle: String {
        switch onboardingActIndex {
        case 0: return "The Curse Proves Itself"
        case 1: return "The Book Learns You"
        case 2: return "The Story Bites Back"
        default: return "The Bindery Returns Proof"
        }
    }
    ...
    private var stepDots: some View {
        VStack(alignment: .leading, spacing: 9) {
            HStack(spacing: 7) {
                ForEach(0..<4, id: \.self) { index in
                    Capsule()
                        .fill(
                            index <= onboardingActIndex
                                ? BookPalette.lampGold
                                : BookPalette.teal.opacity(0.30)
                        )
                        .frame(maxWidth: .infinity)
                        .frame(height: index == onboardingActIndex ? 6 : 4)
                }
            }

            Text("SIGNATURE \(onboardingActIndex + 1) OF 4 · \(onboardingActTitle.uppercased())")
            ...
            if canPutRibbonHere {
                Button {
                    ...
                    Label("Put a ribbon here", systemImage: "bookmark.fill")
                        .font(.subheadline.weight(.bold))
                        .frame(maxWidth: .infinity)
                        .padding(.vertical, 9)
                }
        ...
        .accessibilityLabel("Signature \(onboardingActIndex + 1) of 4, \(onboardingActTitle)")
    }

    /// Only offer the ribbon at sewn seams. Everything needed to resume these
    ...
    /// Answers to the mission the Book just set: what is the nearest ordinary
    /// thing *doing* that gives away its opinion about being there.
    ///
    /// Every one obeys one of the two house rules: Give the World the Pen
    /// (the object takes the verb and acts on the reader) or Cross the Wires
    /// (one sense answers for another). A preset that reads "I noticed the
    /// cup" teaches the reader that the correct answer is a flat report, and
    /// then every page they write afterwards is a flat report.
    private var firstPressPresets: [FirstPressPreset] {
        ...
        switch hour {
        case 5..<12:
            skySentence = "The window has been holding the same rectangle of light hostage since six."
            roomSentence = "The room still tastes faintly of somebody else's sleep."
            bodySentence = "The floor is colder than it has any right to be, and it knows it."
            objectSentence = "The mug is sulking about being empty."
        case 12..<17:
            skySentence = "The afternoon light is leaning on the wall like it has nowhere else to be."
            roomSentence = "This room sounds beige."
            bodySentence = "The chair has been arguing with my spine for an hour and winning."
            objectSentence = "The pen is pretending it didn't roll there on purpose."
        case 17..<22:
            skySentence = "The window has gone the colour of a held breath."
            roomSentence = "The lamp is doing all the work in here, and it knows it."
            bodySentence = "The doorway keeps catching me on the way past, like it wants a word."
            objectSentence = "The kettle ticks like it's counting something down."
        default:
            skySentence = "The dark outside is pressing its face against the glass."
            roomSentence = "The quiet in here has a texture, and it's velvet gone slightly wrong."
            bodySentence = "The bed has been waiting up for me and it isn't pleased."
            objectSentence = "The cable has arranged itself into a shape that clearly means something."
        }
        return [
    ...
    private var livingInkProofLine: String {
        firstPressPreviewDetail(from: firstPressTrimmedText).nonEmpty
            ?? firstPressSouvenirLine
    }

    private var livingInkProofParts: (before: String, word: String, after: String) {
        let tokens = livingInkProofLine.split(whereSeparator: \.isWhitespace).map(String.init)
        guard !tokens.isEmpty else { return ("", "word", "") }
        let stopWords: Set<String> = [
            "a", "an", "and", "at", "but", "for", "from", "i", "in", "is",
            "it", "my", "of", "on", "the", "this", "to", "was", "with", "you"
        ]
        let chosenIndex = tokens.indices
            .filter { index in
                let cleaned = tokens[index]
                    .lowercased()
                    .trimmingCharacters(in: .punctuationCharacters)
                return cleaned.count >= 4 && !stopWords.contains(cleaned)
            }
            .max { left, right in
                tokens[left].count < tokens[right].count
            }
            ?? max(0, tokens.count / 2)
        let beforeWords = tokens[..<chosenIndex]
        let afterWords = tokens[(chosenIndex + 1)...]
        return (
            beforeWords.isEmpty ? "" : beforeWords.joined(separator: " ") + " ",
            tokens[chosenIndex],
            afterWords.isEmpty ? "" : " " + afterWords.joined(separator: " ")
        )
    }
    ...
    private var stepContent: some View {
        if step == 0 {
            ...
            case 0:
                ...
                // reader had never heard plus a rule the Book had already
                // stated twice — which made Zara arrive as a narrator with a
                // face instead of a person.
                onboardingProse("""
                "Don't stand up yet," she says. "Everyone stands up too fast."

                She looks at you like a delivery she wasn't expecting. "You're from outside. The Great Unwritten, we call it. Nobody in here has ever been."
                """)
                ...
                if didSteadyFirstPage {
                    VStack(alignment: .leading, spacing: 18) {
                        ...
                        "That's what makes you the reader," she says. "Your life out there can change what happens in here. Nothing else can do that."

                        She looks at the Page in your hand, and at the grey nick already bitten out of its edge. "So it's been talking to you, then. It does that. It's been alone with the Rut for a very long time."

                        "Come on. There's a whole Academy in here that argues about people like you."

                        She's already walking. "Welcome to the conspiracy, by the way. It lasts as long as your life does."
                        """)
                        continueButton("Follow her in")
                    }
            ...
            case 1:
                guidePortrait(mood: "Zara has an urgent question. It isn't a serious one.")
                ...
                She's already holding the pencil. "Big questions later. I want the small stuff."
                """)
            ...
            case 3:
                onboardingTitle("What Should the Book Call You?")
                onboardingProse("""
            ...
            case 4:
                ...
                "That's the Rut," Zara says. "It turns a whole life into wallpaper. It goes quiet around something you actually believe."
                ...
                if let answer = belief.trimmingCharacters(in: .whitespacesAndNewlines).nonEmpty {
                    ...
                    if investedBelief {
                        onboardingPreviewCard(
                            ...
                            body: "It'll turn up again. In your Pages, in moments that agree with it, in where the story goes next."
                        )
                        continueButton("Good")
                    } else {
            ...
            case 6:
                ...
                onboardingProse("""
                "Not what photographs well," Zara says. "What puts a charge through you."

                "First honest answer. Don't think about it."
                """)
                ...
                if !magicSource.isEmpty {
                    ...
                    onboardingTitle("And Then What Do You Do With It?")
                    // Not "Last one": seven more questions follow this screen,
                    // and a broken promise from the most reliable character in
                    // the flow costs more than the line is worth.
                    onboardingProse("""
                    "One more," Zara says.

                    "You find something good: a line, a light, a whole strange minute. What do your hands do with it?"
                    """)
                    onboardingChoiceList(choices: handsChoices, selection: $roleHands)
                }
                continueButton(
                    "That's my kind",
            ...
            case 7:
                onboardingPagewrightInterlude

            case 8:
                onboardingRoleReveal

            case 9:
                // "Turn up" where? The reader has not seen the desk yet, so
                // there is no surface in their head for things to turn up on.
                onboardingTitle("What Should the Book Bring You First?")
                onboardingProse("""
                Blank cards sort themselves into six impatient piles.

                "They're waiting to see what you want," Zara says.

                A fresh line scratches itself across the nearest card.

                "Waiting? I'm starving."

                "Pick what you're hungry for today."
                """)
            ...
            case 10:
                // Zara's own line below names the setting exactly; the title
                // used to leave the reader guessing between notifications,
                // tone, and content.
                onboardingTitle("How Hard May It Knock?")
                onboardingProse("""
                The grey bite at the Page's edge twitches. The paper pins it back down in the margin.
                ...
                continueButton("That's the rule", disabled: comfortBoundary.isEmpty)

            case 11:
                // They are places with banners, not people, and "it" was the
                // reader's page: two ambiguities in five words.
                onboardingTitle("Five Arguments Want Your Page")
                onboardingProse("""
                Five banners lean toward the answer still warm in your hands.

                "These aren't teams, and they're definitely not houses with nicer robes," Zara says. "They're five arguments about what makes an ordinary life worth anything. They don't agree. They never have."
            ...
            case 12:
                // He has not heard an argument to disagree with: he has taken
                // the Page out of the reader's hand and is walking backwards
            ...
            default:
                ...
                if didReadInscriptionWriteback {
                    ...
                    case 0:
                        ...
                        onboardingConclusionButton("Bind what I made") {
                        ...
                        if didBindFirstEdition {
                            onboardingPreviewCard(
                                ...
                                body: didCloseFirstEditionPreview
                                    // Not "the Standing Order can't take it
                                    // away": the paywall is offered after
                                    // onboarding completes, so this reassured
                                    // the reader about a product they had
                                    // never heard of, and planted the idea
                                    // that something here might be taken.
                                    ? "It's saved in the Book's own files. Read it, share it, print it, or keep it. It's yours whatever you do next."
                            ...
                            if didCloseFirstEditionPreview {
                                onboardingConclusionButton("One last thing") {
                            ...
                            // Every other step carries the "Open the free Book
                            // now" escape, but it is hidden on the last one:
                            // so a binding failure that repeats had no way
                            // forward and no way out. The reader's answers are
                            // already in `onboardingResult`; the edition is the
                            // only thing missing, and it can be bound later
                            // from the Bindery.
                            onboardingConclusionButton("Carry on; bind it later") {
                                conclusionMiniStep = 2
                            }
                        }
                    ...
                    default:
                        onboardingThirtyDayPromise
                            .modifier(OnboardingSectionArrivalModifier(kind: .artifact, delay: 0.06))
                        continueButton("Open the Book", disabled: whisperCadence.isEmpty)
                    }
                } else {
                    onboardingProse("""
                    Wicker holds the Page a moment longer than he needs to. Then he flips it back at you, and you catch it.

                    "Go on then," he says. "See what it did with you."
    ...
    private var onboardingSymbol: String {
        ...
        case 8: return "seal"
        case 9: return "rectangle.stack.badge.play"
        case 10: return "scalemass"
        case 11: return "flag.2.crossed"
        case 12: return "theatermasks"
    ...
    private var onboardingStageName: String {
        ...
        case 7: return "The Pagewright"
        case 8: return "What You Are"
        case 9: return "First Pages"
        case 10: return "How Hard to Push"
        case 11: return "Five Arguments"
        case 12: return "A Consequence"
    ...
    private var onboardingHeaderLine: String {
        ...
        case 7: return "Your answers stop being answers and become a Page."
        case 8: return "It has read enough, including what your hands just made."
        case 9: return "What you'd like to hear from first."
        case 10: return "How hard the story is allowed to push."
        case 11: return "Five old arguments, disagreeing about you."
        case 12: return "Somebody in here remembers what you chose."
    ...
    private var openingMiniStageName: String {
        ...
        case 1: return "The Ink Cheats"
        case 2: return "The Grey Bite"
        case 3: return "The Awake Thing"
        case 4: return "The Curse"
        case 5: return "The Conspiracy"
        case 6: return "Your First Move"
        case 7: return "Two Honest Answers"
    ...
    private var openingMiniSymbol: String {
        ...
        case 6: return "sparkle.magnifyingglass"
        case 7: return "text.bubble.fill"
        default: return "sparkles"
        }
    }


    /// Three questions the reader was present for and cannot answer. "Gone" is
    /// exactly as easy to tap as "I remember it": the beat only works if the out
    /// is effortless and the Book sounds pleased rather than sorry about it.
    @ViewBuilder
    private var onboardingRecallProbes: some View {
        VStack(alignment: .leading, spacing: 10) {
            ForEach(Array(recallQuestions.enumerated()), id: \.offset) { index, question in
                let state = recallState(index: index)
                VStack(alignment: .leading, spacing: 8) {
                    Text(question)
                        .font(.system(.callout, design: .serif))
                        .foregroundStyle(BookPalette.ink.opacity(0.86))
                    ...
                    HStack(spacing: 8) {
                        recallAnswerButton("I remember it", index: index, remembered: true, selected: state == true)
                        recallAnswerButton("Gone", index: index, remembered: false, selected: state == false)
                    }
                }
                .padding(12)
                .frame(maxWidth: .infinity, alignment: .leading)
                ...
                .overlay {
                    RoundedRectangle(cornerRadius: 9, style: .continuous)
                        .stroke(BookPalette.ink.opacity(state == nil ? 0.12 : 0.24), lineWidth: 1)
                }
                .modifier(OnboardingSectionArrivalModifier(kind: .section, delay: 0.05 + Double(index) * 0.07))
            }
        }
    }

    private func recallAnswerButton(_ title: String, index: Int, remembered: Bool, selected: Bool) -> some View {
        Button {
            answerRecall(index: index, remembered: remembered)
        } label: {
            Text(title)
                .font(.subheadline.weight(.bold))
                .frame(maxWidth: .infinity)
                .padding(.vertical, 9)
                .background(
                    (selected ? (remembered ? BookPalette.teal : BookPalette.ink) : BookPalette.paper)
                        .opacity(selected ? 0.22 : 0.5),
                    in: RoundedRectangle(cornerRadius: 8, style: .continuous)
                )
                .overlay {
                    RoundedRectangle(cornerRadius: 8, style: .continuous)
                        .stroke(
                            (selected ? (remembered ? BookPalette.teal : BookPalette.ink) : BookPalette.ink).opacity(selected ? 0.5 : 0.14),
                            lineWidth: 1
                        )
                }
        }
        .buttonStyle(.bookPress())
        .foregroundStyle(selected ? BookPalette.ink : BookPalette.ink.opacity(0.7))
    }

    /// The "I caught it" answer is the dangerous one: a reader who beat the
    /// trick can decide the whole premise is a party game and stop listening.
    /// So the Book doesn't congratulate them: it demonstrates the same
    /// machinery a second time, on them, using the sentence they are currently
    /// reading. Prediction is why the word vanished unnoticed, and it is also
    /// why nobody reads letters. Naming the mechanism turns a trick into an
    /// argument.
    private var livingInkPredictionProse: String {
        let opener: String
        switch pageChangeNoticed {
        case "caught":
            opener = "\"Good. You're one of the few. Now read this.\""
        case "uncertain":
            opener = "\"Something did. Read this.\""
        default:
            opener = "\"Nobody does. Read this.\""
        }
        return """
        \(opener)

        "You don't read words. You read shapes, and your head fills in the rest. You don't see the letters at all, not until I've sepllt something wrong."

        "There. Now you can't unsee it."

        "Same machinery took your word. Your head is fast and lazy. It's been eating your Tuesdays for years."
        """
    }

    /// "That's yours now" left the reader to work out what "that" was, at the
    /// point where the Book should be showing it can hold a specific thing.
    /// The claim to *have* it is only made when they actually kept it:
    /// telling somebody who let the Page wait that the Book has their moment
    /// would be a straight lie about the choice they just made.
    private var momentFateOpeningProse: String {
        let held = rehearsalChoice == .keep ? " I have them." : ""
        return """
        "That moment, that sentence: they're yours now, and they're safe where they are.\(held)"

        "The rest of them don't do so well. What usually happens to small things like that one?"
        """
    }

    /// The verdict has to answer what actually happened. A reader who kept all
    /// three has just disproved the opening claim, and telling them they
    /// weren't there would be both wrong and insulting: the Book would rather
    /// be caught out than keep reciting.
    private var openingVerdictTitle: String {
        recallRemembered >= 3 ? "You've Made This Awkward" : "That's the Curse"
    }

    /// Names both halves of the contrast outright. Saying "one of them" and
    /// "the other" made the reader do the bookkeeping at the exact moment the
    /// argument lands, and an argument the reader has to reconstruct is one
    /// they can decline.
    private var openingVerdictProse: String {
        let hour = RoutineRecallProbe.scenario(id: recallScenario)?.noun ?? "the ordinary hour"
        let awake = mostAliveTitle.lowercased()
        let detail = awakeMemory.trimmingCharacters(in: .whitespacesAndNewlines).nonEmpty
        let handed = detail.map { "you handed me \u{201C}\($0)\u{201D} without stopping to think" }
            ?? "it came back to you immediately"

        if recallRemembered >= 3 {
            return """
            "Same week, same skull, and you were there for the \(hour) *and* for \(awake). Do you know how uncommon that is?"

            "I came in here ready to prove something. You've spoiled it. I don't mind."

            "The Curse is real. It just hasn't got as much of you yet."
            """
        }
        return """
        "There. Same week, same skull. The \(hour) is gone: you were awake for every minute of it and you can't get back inside it now."

        "Then I asked about \(awake), and \(handed)."

        "You weren't there for the \(hour). That's all it takes."
        """
    }

    private var openingOfferProse: String {
        if recallRemembered >= 3 {
            return """
            "I can't lift it. I'm a book: no hands, and I can't make you look at anything."

            "But I notice everything, I never get bored, and somebody who still turns up to their own life is exactly who I've been waiting to catch."

            "So. Where does it get in anyway?"
            """
        }
        return """
        "I can't lift the Curse. I'm a book: no hands, and I can't make you look at anything."

        "But I notice everything, I never get bored, and I've been waiting a long time for somebody to hand me something true."

        "So. Where does it get in worst?"
        """
    }

    // MARK: The interrogation

    /// Seeded off the arrival moment so the same reader is always asked the
    /// same three. Being asked different questions on a second look would show
    /// the machinery and cost the beat its authority.
    private var recallQuestions: [String] {
        RoutineRecallProbe.questions(
            scenarioID: recallScenario,
            seed: "\(Int(arrivalTimestamp))-\(recallScenario)"
        )
    }

    private var recallAnswers: [Character] { Array(recallAnswersRaw) }

    private var recallAnswered: Bool {
        !recallScenario.isEmpty && recallAnswersRaw.count == recallQuestions.count
    }

    private var recallRemembered: Int {
        recallAnswers.filter { $0 == "1" }.count
    }

    private func answerRecall(index: Int, remembered: Bool) {
        var chars = Array(recallAnswersRaw)
        while chars.count <= index { chars.append("-") }
        chars[index] = remembered ? "1" : "0"
        BookFeedback.play(.select)
        withAnimation(.spring(response: 0.3, dampingFraction: 0.82)) {
            recallAnswersRaw = String(chars.prefix(recallQuestions.count))
        }
    }

    private func recallState(index: Int) -> Bool? {
        guard index < recallAnswers.count else { return nil }
        switch recallAnswers[index] {
        case "1": return true
        case "0": return false
        default: return nil
        }
    ...
    /// The tally, stored as the routine-memory baseline the rest of the app
    /// already reads. A count of concrete misses is harder evidence than the
    /// old self-assessment, which people flattered themselves answering.
    private var recallBaselineID: String {
        switch recallRemembered {
        case 0: return "blank"
        case 1: return "result"
        case 2: return "islands"
        default: return "full"
        }
    }
    ...
    private var onboardingOpeningFindsYouStep: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            openingMiniContinueButton("Put your thumb on it") {
    ...
    private var onboardingOpeningLivingInkStep: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            if didRevealRoutineEvidence {
                onboardingRoutineReceiptCard
                    .transition(.opacity.combined(with: .move(edge: .top)))
                openingMiniContinueButton("Ask me about them") {
                    advanceOpeningMiniStep()
                }
    ...
    private var onboardingOpeningRecallStep: some View {
        VStack(alignment: .leading, spacing: 16) {
            onboardingTitle("Something You Did Today")
                .id("onboarding-opening-recall")
            // Picks up the receipt from the previous beat by its own number, so
            // the two screens read as one argument rather than a fresh topic.
            onboardingProse("""
            "Then let's take one hour out of the \(openingSpentMinutesToday). Which of these did you do last? Don't think about it."
            """)
    ...
    private var onboardingOpeningAwakeStep: some View {
        VStack(alignment: .leading, spacing: 16) {
            onboardingTitle("The Last Time You Were Wide Awake")
                .id("onboarding-opening-control")
            // The chips answer "what were you doing", so the question has to
            // ask that. It used to ask "when", leaving the reader to reconcile
            // a time question against eight activity answers.
            //
            // "The last time, not your favourite time" blocks the idealised
            // holiday answer: the rest of the flow needs something from this
            // month, not the best week of their life.
            onboardingProse("""
            "What were you doing? The last time, not your favourite time."
            """)
            onboardingChipChoices(choices: aliveChoices, selection: $mostAlive)
            if !mostAlive.isEmpty {
                onboardingProse("""
                "Good. Now go back into it. One thing you can still see, hear, or feel. A sound. A colour. What your hands were on."

                "No summaries. I'll know."
                """)
                onboardingField("One exact thing...", text: $awakeMemory)
            }
            openingMiniContinueButton(
                "Hand it over",
    ...
    private var onboardingOpeningConspiracyStep: some View {
        VStack(alignment: .leading, spacing: 16) {
            // Not "What We're Up Against": there is no "we" until the
            // arrangement below creates one.
            onboardingTitle("What It's Called")
                .id("onboarding-opening-offer")
            // Three names, not five. The aliases exist because the rest of the
            // app really does say Rut in code and Grey on the Page, so the
            // reconciliation has to happen — but a five-item glossary on minute
            // six teaches the reader that the Book cannot decide what its own
            // antagonist is called.
            onboardingProse("""
            "I call it the Curse. Other things in here call it the Rut, or the Grey. Same thing."
            """)
            ...
            if !rutStrongest.isEmpty {
                ...
                onboardingProse("""
                "I've been fighting this from inside these covers longer than I can remember. I've never once got out. I can't reach your world. You can walk around in it."
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            if firstPressTrimmedText.isEmpty {
                ...
                onboardingOpeningKeepDecision
                    .transition(.opacity.combined(with: .move(edge: .top)))
                // The Rut's entrance branches on the keep-or-wait choice made
                // in the card directly above, so it has to render after it.
                // It used to sit at the very top of the step, which meant the
                // antagonist's first physical act appeared above content the
                // reader had already scrolled past, and effectively never
                // played.
    ...
    private var onboardingOpeningHonestAnswersStep: some View {
        VStack(alignment: .leading, spacing: 16) {
            onboardingTitle("What Happens To The Others")
            ...
            onboardingTitle("The Thing You Just Wrote Down")
                .id("onboarding-opening-hidden-magic")
            // Not "that ordinary object you went and found": the prompt only
            // suggested an object, and a reader who tapped the sky preset or
            // wrote about their mood would be told they did something they
            // didn't. The Book must not describe an act it cannot see.
            onboardingProse("""
            "I want the true answer, not the polite one."

            "When you looked at it twice: did it get stranger, or did it stay exactly as dull as it was?"
            """)
    ...
    //
    // Three rules for every line the Book speaks. It has drifted twice, both
    // times into the same place, so they live next to the copy now:
    //
    //   1. Never explain its own line. No "that's not X, it's Y". A clever
    //      feral thing asserts and moves on; annotation is what an assistant
    //      does with a sentence it does not trust.
    //   2. Never manage the reader's feelings. No permission-giving, no
    //      softening a hard result, no "don't feel caught out". The Book is
    //      pleased by what it finds, never sorry about it.
    //   3. Never make its own claim smaller to seem reasonable. Talking the
    //      curse down to something modest and defensible is servant instinct.
    //
    // And never narrate itself in the third person: no "the Book says". It is
    // speaking; attribution is for characters who aren't.

    /// The very first words of the whole app. Deliberately ordinary and
    /// physical: the reader's own hands, doing a thing anybody could do. The
    /// strangeness belongs on the Page that falls out, not in the narration:
    /// an app that opens by shouting at you has spent its shock before the
    /// reader has touched anything.
    static let openingFirstWords = "You pick up the book you just found. Nobody saw you take it."

    /// Overprinted across the loose Page. This is where the curse warning
    /// lives now: the Page arrives already carrying it, so the claim is a thing
    /// the reader finds rather than a thing the app declares at them.
    static let openingPageStamp = "STOP. You're under a curse. I can prove it."
    ...
    private var onboardingOpeningPullQuote: some View {
        HStack(alignment: .top, spacing: 10) {
            ...
            Text("A page falls out. It's about today. Today hasn't finished happening yet.")
                .font(.system(.title3, design: .serif).weight(.semibold))
                .foregroundStyle(BookPalette.ink.opacity(0.82))
                .lineSpacing(3)
                .fixedSize(horizontal: false, vertical: true)
    ...
    private var onboardingNoticedLine: some View {
        HStack(alignment: .top, spacing: 9) {
            ...
            Text(firstPressTrimmedText.isEmpty
                ...
                : "That sentence was happening before I asked. Routine had filed it as scenery. You just made it an event.")
                .font(.system(.callout, design: .serif))
    ...
    private var onboardingOpeningPress: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(alignment: .top, spacing: 8) {
                ...
                Text("Look away from me. Find the nearest ordinary thing around you with an opinion about being there. What small thing is it doing that gives it away?")
            ...
            Text("Write one sentence, change one of mine, or leave it exactly as it is. I'll take an unimpressed answer over a decorative one.")
                .font(.caption.weight(.semibold))
                .foregroundStyle(BookPalette.ink.opacity(0.58))
                .fixedSize(horizontal: false, vertical: true)
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 9) {
                ...
                onboardingPactPromise(
                    ...
                    // Not "more of your life back than the month before": that
                    // guarantees monotonic improvement, and the first month it
                    // fails to hold the Book is caught lying on the one card
                    // where it is asking to be trusted.
                    text: "Every month I hand you back what you kept, in writing. You can check it against what you remember and tell me if I'm lying."
                )
            }

            Text("\"Now yours. Come back when the day has room. Bring me one true thing. Play when a door opens. That's all of it. I don't keep attendance. I keep Pages.\"")
                .font(.system(.callout, design: .serif))
                .foregroundStyle(BookPalette.ink.opacity(0.82))
                .lineSpacing(2)
                .fixedSize(horizontal: false, vertical: true)
    ...
    private var onboardingFirstPressScene: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack(alignment: .top, spacing: 11) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text("THE PAGE WANTS TO KNOW SOMETHING")
                        .font(.system(size: 10, weight: .black))
                        .tracking(1.1)
                        .foregroundStyle(BookPalette.gold)
                    // Not "A small narrative decision": that is the widget's
                    // design label, set in the Book's own serif as though the
                    // Book had said it.
                    Text("How should it tell your day?")
                        .font(.system(.title3, design: .serif).weight(.semibold))
                        .foregroundStyle(BookPalette.ink)
                        .fixedSize(horizontal: false, vertical: true)
                }
            ...
            Text("""
            The loose Page lifts. "I can leave our story about the little everyday things, bend it toward what happens next, or let the margin surprise us both."
    ...
    private var onboardingOpeningKeepDecision: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text("Keep it and I read it. Tonight, and after. It comes back in stories, in letters, in things I notice about you weeks from now that you\u{2019}ll swear you never told me.\n\nLet it wait and nothing is lost. I\u{2019}ll note that you didn\u{2019}t. That tells me something too.")
                .font(.system(.callout, design: .serif))
                .foregroundStyle(BookPalette.ink.opacity(0.72))
                .fixedSize(horizontal: false, vertical: true)
            ...
            if rehearsalChoice == .wait {
                Text("The Page folds the sentence inward and says nothing about it.")
                    .font(.system(.callout, design: .serif).italic())
                    .foregroundStyle(BookPalette.ink.opacity(0.68))
                    .fixedSize(horizontal: false, vertical: true)
                    .transition(.opacity.combined(with: .move(edge: .top)))
    ...
    private var onboardingOpenBookTab: some View {
        Button {
            ...
            HStack(spacing: 10) {
                Image(systemName: "book.closed.fill")
                    .font(.system(size: 16, weight: .black))
                Text(rehearsalChoice == nil ? "Choose what the Page should do" : "Open the Book")
    ...
    private var onboardingOpeningArtifact: BookOfYouShareArtifact {
        ...
        return BookOfYouShareArtifact(
            ...
            // The warning stays when the reader writes their own first line and
            // the rest of the Page re-forms around it. The Page becomes theirs;
            // what it came to tell them does not get overwritten by them.
            stamp: Self.openingPageStamp
        )
    }
    ...
    private var onboardingInscriptionBookOfYouArtifact: BookOfYouShareArtifact {
        ...
        let thresholdActs = onboardingPagewrightIsReady
            ? "\(decision), made a Page from the loose evidence, and watched \(chapter) change what happened next"
    ...
    /// The opening's entire argument, closed at the end: they could not
    /// retrieve three ordinary things from an hour they lived through, and then
    /// handed over one exact thing without pausing. Quoting their own words
    /// back is the proof: anything the Book asserts here it has already shown.
    private var recallContrastLine: String {
        let detail = awakeMemory.trimmingCharacters(in: .whitespacesAndNewlines)
        let scenarioName = RoutineRecallProbe.scenario(id: recallScenario)?.label.lowercased()
            ?? "an ordinary hour"
        guard let detail = detail.nonEmpty else {
            return "You went looking for the ordinary hour and came back with almost nothing. That wasn't a failure of memory. You simply weren't there for it."
        }
        let missed = RoutineRecallProbe.askedCount - recallRemembered
        if missed <= 0 {
            return "You kept all three small things from when you \(scenarioName). Then I asked for one thing from a moment you were awake for, and you handed me \u{201C}\(detail)\u{201D} without stopping to think. The study isn't your verdict. Your own evidence says you were there."
        }
        let missedClause = "\(missed) of the three small things I asked about were gone"
        return "\(missedClause) from when you \(scenarioName). Then I asked for one thing from a moment you were awake for, and you handed me \u{201C}\(detail)\u{201D} without stopping to think. Same memory. Same week. You were only in one of them."
    }
    ...
    private var onboardingPersonalizedReturn: some View {
        ...
        return VStack(alignment: .leading, spacing: 13) {
            ...
            onboardingJourneyRow(
                ...
                detail: "\(decision) The margin tucked \(ration) beside it: a fact I couldn't have guessed."
            )
            onboardingJourneyRow(
                symbol: "eye.trianglebadge.exclamationmark",
                title: "The proof is in your own handwriting",
                detail: recallContrastLine
            )
            onboardingJourneyRow(
                symbol: "scope",
                title: "I learned where to look",
                detail: "I'll watch for the Rut around “\(rutTitle.lowercased()),” and for signs of life in “\(mostAliveTitle.lowercased())” and “\(magicSourceTitle.lowercased()).”"
            )
            onboardingJourneyRow(
                symbol: "person.crop.circle.badge.checkmark",
                title: investedBelief ? "\"\(planted)\" joined the Cast" : "The answer is saved",
                detail: investedBelief
                    ? "You lent it Belief. Now I can bring it back whenever something in your life agrees with it."
            ...
            // The panel ends on the name it earned. It used to summarise the
    ...
    private var onboardingThirtyDayPromise: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    Text("BEFORE I LET YOU GO")
                        .font(.system(size: 12, weight: .black))
                        .tracking(1.1)
                        .foregroundStyle(BookPalette.gold)
                    Text("Choose when I may tap the glass.")
                        .font(.system(.title3, design: .serif).weight(.bold))
                        .foregroundStyle(BookPalette.ink)
                }
            }

            // One statement of the loop. "Here is the trick. Catch. Keep or
            // refuse. Watch what comes back." was the same three beats in a
            // different phrasing, one line above this.
            ...
            VStack(alignment: .leading, spacing: 10) {
                Text("WHEN SHOULD I KNOCK?")
                    .font(.system(size: 12, weight: .black))
                    .tracking(1.0)
                    .foregroundStyle(BookPalette.gold)
                Text("A new habit sticks best when it sits beside one you already have.")
                    .font(.system(.body, design: .serif).weight(.semibold))
                    .foregroundStyle(BookPalette.ink.opacity(0.78))
                    .fixedSize(horizontal: false, vertical: true)
                onboardingChipChoices(
                ...
                if !whisperCadence.isEmpty {
                    ...
                    Text("Pick one to carry on. Silence counts too; if you leave it blank, I will chew the ribbon instead.")
            ...
            Text("Missing days doesn't undo it. Almost nobody builds one cleanly.")
                .font(.system(.body, design: .serif).weight(.semibold))
                .foregroundStyle(BookPalette.ink.opacity(0.72))
                .fixedSize(horizontal: false, vertical: true)
            ...
            Text("Give it one chance a day. Thirty days from now you'll have up to thirty specific moments the blur would have eaten.")
                .font(.system(.body, design: .serif))
                .foregroundStyle(BookPalette.ink.opacity(0.68))
                .fixedSize(horizontal: false, vertical: true)
    ...
    private var onboardingDayThirtyAfterCard: some View {
        VStack(alignment: .leading, spacing: 13) {
            ...
            Text("You may still have the same inbox, dishes, aches, and unfinished problems. But you'll have caught the moon between two buildings, the joke somebody nearly swallowed, the exact Tuesday you felt like yourself again \u{2014} and stayed long enough to write it down.")
                .font(.system(.body, design: .serif))
                .foregroundStyle(BookPalette.nightText.opacity(0.78))
                .fixedSize(horizontal: false, vertical: true)
            ...
            Text("Your ordinary life stops getting away from you.")
    ...
    private var onboardingHabitScienceCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("Thirty doors. Some will stay shut. The ones you open teach you where to look.")
                .font(.system(.body, design: .serif).weight(.semibold))
            ...
            onboardingHabitScienceStat(
                ...
                body: "This is the part people get wrong. Missing a day didn't set anyone in the study back, and the people who did build the habit missed plenty of days along the way. Patchy, uneven, on-and-off is not the failed version of this. It is the normal one, and it still works."
            )
    ...
    private var onboardingInscriptionCoverChoice: some View {
        VStack(alignment: .leading, spacing: 11) {
            HStack(alignment: .top, spacing: 10) {
                Image(systemName: "book.closed.fill")
                    ...
                    .foregroundStyle(BookPalette.nightText)
                    .frame(width: 34, height: 34)
                    .background(BookPalette.violet.opacity(0.92), in: Circle())
                ...
                VStack(alignment: .leading, spacing: 3) {
                    Text("CHOOSE ITS COAT")
                        ...
                        .tracking(1.05)
                        .foregroundStyle(BookPalette.gold)
                    Text("\"Pick my face. I can't see it while I'm open.\"")
                        .font(.system(.callout, design: .serif).weight(.semibold))
                        .foregroundStyle(BookPalette.ink.opacity(0.80))
                        .fixedSize(horizontal: false, vertical: true)
                }
            ...
            Text("The thread-and-door sigil belongs only to this first edition. The other four are pictures kept beside the Bindery press. If you placed a photograph inside, it may come to the cover too. Your words stay exactly the same.")
                .font(.caption.weight(.semibold))
                .foregroundStyle(BookPalette.ink.opacity(0.62))
                .fixedSize(horizontal: false, vertical: true)
            ...
            if let proof = inscriptionSelectedCoverProofImage {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Text("This is the actual crop, line fitting, ink, and title-safe clearing the PDF will use.")
                        .font(.caption2)
                        .foregroundStyle(BookPalette.ink.opacity(0.58))
                        .fixedSize(horizontal: false, vertical: true)
                }
                .padding(10)
                .background(BookPalette.nightPanel.opacity(0.07), in: RoundedRectangle(cornerRadius: 9))
            }
            #endif
        ...
        .background(
            LinearGradient(
                colors: [
                    BookPalette.page.opacity(0.78),
                    BookPalette.violet.opacity(0.08),
    ...
    private func inscriptionCoverChoiceButton(
        ...
        return Button {
            ...
            VStack(alignment: .leading, spacing: 6) {
                inscriptionCoverChoiceArtwork(id: id, plate: plate, readerImage: readerImage)
                    ...
                    .overlay {
                        RoundedRectangle(cornerRadius: 8, style: .continuous)
                            .stroke(
                                isSelected ? BookPalette.lampGold.opacity(0.92) : BookPalette.ink.opacity(0.14),
                    ...
                    .overlay(alignment: .topTrailing) {
                        if isSelected {
                            Image(systemName: "checkmark.seal.fill")
                                ...
                                .foregroundStyle(BookPalette.nightText)
                                .padding(6)
                                .background(BookPalette.lampGold.opacity(0.94), in: Circle())
                                .padding(6)
                        }
                    }
            ...
            .padding(7)
            .background(
                isSelected ? BookPalette.lampGold.opacity(0.13) : BookPalette.page.opacity(0.58),
                in: RoundedRectangle(cornerRadius: 9, style: .continuous)
            )
        }
        .buttonStyle(.plain)
    ...
    private var inscriptionSelectedCoverProofImage: UIImage? {
        ...
        let copy = MonthlyEditionPDFWriter.VolumeCoverCopy(
            readerName: name.trimmingCharacters(in: .whitespacesAndNewlines).nonEmpty ?? "Reader",
            coverLine: "THE FIRST DOOR",
            coverSubline: BoundReaderRole(namedRole)?.markedName ?? namedRole.fullName
        )
    ...
    private func inscriptionCoverChoiceArtwork(
        ...
        if id == "reader-photo", let readerImage {
            Image(uiImage: readerImage)
                .resizable()
                .scaledToFill()
        } else if let plate {
            #if canImport(UIKit)
            ...
            ZStack {
                LinearGradient(
                    colors: [BookPalette.nightPanel, BookPalette.violet.opacity(0.86)],
                    startPoint: .top,
                    endPoint: .bottom
                )
                VStack(spacing: 8) {
    ...
    private var onboardingFirstBindingInvite: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(alignment: .top, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 5) {
                    // Not "already working": nothing is sewn until the reader
                    // taps below.
                    Text("THE BINDERY")
                        .font(.system(size: 10, weight: .black))
                        .tracking(1.1)
                        .foregroundStyle(BookPalette.gold)
                    Text(didBindFirstEdition ? "Your first edition's bound." : "There's enough here to bind a first edition.")
                        .font(.system(.title3, design: .serif).weight(.semibold))
                        .foregroundStyle(BookPalette.ink)
                        .fixedSize(horizontal: false, vertical: true)
                    Text(
                        didKeepOnboardingPhoto
                            ? "A real PDF with a cover, the Pagewright leaf you made, your illuminated photograph, the Chapter's reading, Wicker's consequence, the Book's write-back, and a letter from Zara. Keep it, share it, print it."
                            : "A real PDF with a cover, the Pagewright leaf you made, the Chapter's reading, Wicker's consequence, the Book's write-back, and a letter from Zara. Keep it, share it, print it."
                    )
                        .font(.caption.weight(.semibold))
                        .foregroundStyle(BookPalette.ink.opacity(0.62))
                        .fixedSize(horizontal: false, vertical: true)
    ...
    /// Synthesizes a real special edition from the onboarding answers. Genuine
    /// `BookPage`s still pass through the shared curator so no kept material is
    ...
    private func inscriptionEdition(sampleBookOfYouPageURL: URL? = nil) -> MonthlyEdition {
        ...
        var edition = MonthlyEditionBuilder.edition(
            ...
            readerRole: BoundReaderRole(namedRole),
        ...
        edition.sections = edition.sections
            .compactMap { section -> MonthlyEditionSection? in
                ...
                case "souvenirs":
                    ...
                    section.note = "The answers, choices, and small human facts that made this copy yours."
                case "images":
                    section.title = "One Piece of Your World"
                    section.note = "A real thing from outside the Book, pressed into the binding."
                case "letters":
                    section.title = "A Letter from the Sentence-Dust"
                    section.note = "Zara Finch writes from the Academy side of the door."
                default:
                    break
                }
                return section
        ...
        let curseTitle: String
        let curseEvidence: String
        let curseMeaning: String
        if recallAnswered, forgotten == 0 {
            curseTitle = "The Curse Meets Its Exception"
            curseEvidence = "You kept all three small details from the \(RoutineRecallProbe.scenario(id: recallScenario)?.noun ?? "ordinary hour")."
            curseMeaning = "The Rut didn't catch that minute. Good. A curse worth naming has to survive being contradicted."
        } else if recallAnswered {
            curseTitle = "The Curse Proves Itself"
            curseEvidence = "\(forgotten) of three small details were gone from the \(RoutineRecallProbe.scenario(id: recallScenario)?.noun ?? "ordinary hour")."
        ...
        let role = BoundReaderRole(namedRole)
        let chapterName = drawnChapter?.name ?? "The Five Chapters"
        let chapterBody = drawnChapter.map { chapterReading(for: $0) }
            ?? "The banners argued over the Page and declined to become a tidy personality test."
        let wickerLine: String = {
            guard let outcome = wickerOutcome,
                  let choice = wickerChoices.first(where: { $0.id == outcome.choiceID }) else {
                return "Wicker left the margin unbitten. For now."
            }
        ...
        let firstDetailWords = firstDetailRaw
            ...
            .filter { !$0.isEmpty }
            .joined(separator: " ")
        let firstDetail = firstDetailWords.count > 180
            ? String(firstDetailWords.prefix(177)).trimmingCharacters(in: .whitespacesAndNewlines) + "…"
            : firstDetailWords
        let thresholdChoiceLine: String = {
            switch rehearsalChoice {
            case .keep:
                return "You told the Book to keep it. The loose Page stayed instead of blowing back under the door."
            case .wait:
                return "You let the offered Page wait. The Book kept the shape of that refusal instead of pretending it was a yes."
            case nil:
                return "The first Page reached the threshold before the Book knew whether it would be kept. The uncertainty stayed in the binding."
            }
        }()
        let pagewrightReturn = onboardingPagewrightIsReady
            ? "The Pagewright pressed your scraps into a leaf, and the Bindery returned the whole crossing as this copy."
            : "The Bindery pressed the Page, the argument, and the bite mark into this copy."
        let bookNote = """
        \(readerName),

        I was blank at this door. Then you gave me \(firstDetail), and look what it has done to my hinges.

        This copy is not a portrait of you. Portraits sit still and pretend they have finished. This is the first thing we made while watching each other: one ordinary detail, one choice, one argument, one bite mark, one thing returned.

        I have only caught you once. I kept the evidence anyway.

        - The Book
        """
        let wickerAtTableLine: String = {
            guard let outcome = wickerOutcome,
                  let choice = wickerChoices.first(where: { $0.id == outcome.choiceID }) else {
                return "I opened it. The margin was suspiciously unbitten, so I have left a little room for trouble."
            }
            return "I opened it. I bit the margin. The reader answered with \(choice.title.lowercased()), and the page had the decency to change afterward."
        }()
        let bindingConversation = BoundVolumeCastConversation(
            ...
            lines: [
                .init(
                    ...
                    glyph: nil,
                    words: "Open it at the hinge. \(firstDetail) is doing the real work. Small enough to be missed, specific enough to hold."
                ),
                .init(
                    id: "first-door-wicker-bite",
                    speakerID: "wicker-eddies",
                    speakerName: "Wicker Eddies",
                    glyph: nil,
                    words: wickerAtTableLine
                ),
                .init(
                    id: "first-door-zara-record",
                    speakerID: "zara-finch",
                    speakerName: "Zara Finch",
                    glyph: nil,
                    words: "Good. The disagreement belongs in the binding. This copy remembers what happened, not the flattering version."
                ),
                .init(
                    id: "first-door-wicker-verdict",
                    speakerID: "wicker-eddies",
                    speakerName: "Wicker Eddies",
                    glyph: nil,
                    words: "Fine. It isn't a portrait. It caught \(readerName) at one door, with one real thing in hand. That's smaller than prophecy and much harder to fake."
                )
            ],
            evidenceIDs: [
                "first-door-detail",
                "chapter:\(drawnChapter?.id ?? "undrawn")",
                "wicker:\(wickerOutcome?.choiceID ?? "unanswered")",
        ...
        let specialClosing = """
        \(readerName), this is not proof that I know your whole life. It is proof that I watched what you actually did at one door: what you noticed, what you let pass, what you defended, and what bit back.

        You came through as \(role?.markedName ?? "a reader still being named"). Bring me another ordinary thing. I want to see whether it grows teeth.
        ...
        edition.inscriptionPublication = InscriptionPublicationMatter(
            ...
            signatures: [
                .init(
                    id: "curse",
                    title: curseTitle,
                    evidence: curseEvidence,
                    meaning: curseMeaning
                ),
                .init(
                    id: "learning",
                    title: "The Book Learns You",
                    evidence: "Most alive: \(mostAliveTitle). First magic: \(magicSourceTitle). Pages first: \(tasteTitle). Opening edge: \(comfortTitle).",
                    meaning: "Not a profile. Manners for a living Book, learned from what you told it."
                ),
                .init(
                    id: "consequence",
                    title: "The Story Bites Back",
                    evidence: "\(chapterName) read your Page. \(wickerLine)",
                    meaning: "Your choice changed the fiction that followed it. The story kept the bruise."
                ),
                .init(
                    id: "return",
                    title: "The Bindery Returns Proof",
                    evidence: onboardingPagewrightIsReady
                        ? "\(firstDetail) became a Page you composed, an Academy argument, a consequence, and this bound copy."
                        : "\(firstDetail) became a Page, an Academy argument, a consequence, and this bound copy.",
                    meaning: "The artifact is the promise made tangible: something from your actual life came back changed."
                )
            ],
            firstArgumentTitle: chapterName,
            firstArgumentBody: "\(chapterBody)\n\n\(wickerLine)",
            ...
            bookNote: bookNote,
            thresholdThread: [
                .init(id: "noticed", stage: "YOU NOTICED", detail: firstDetail),
                .init(id: "kept", stage: "THE BOOK KEPT", detail: thresholdChoiceLine),
                .init(id: "argued", stage: "THE ACADEMY ARGUED", detail: "\(chapterName) took the first reading and refused to call it a verdict."),
                .init(id: "bit", stage: "WICKER BIT THE MARGIN", detail: wickerLine),
                .init(id: "returned", stage: "THE PRESS RETURNED", detail: pagewrightReturn)
            ],
            bindingConversation: bindingConversation,
            readerCoverArtwork: readerCoverArtwork
        )
        edition.foreword = ""
        edition.bindingStory = nil
        edition.closing = specialClosing
        edition.pageCount = edition.sections.reduce(0) { partial, section in
    ...
    private var firstPressTransitionProse: String {
        ...
        case .keep:
            return """
            There. That's all it is. One ordinary detail, pressed flat, and now it can't get away.

            Now the loose page slides back between the covers. The Book opens underneath it.
            """
        case .wait:
            return """
            Good. The page that waits still counts. I want real details, not good ones.

            Now the sample page turns in the air. The cover opens underneath it.
            """
        case nil:
            return """
            One real detail is enough to start with.
    ...
    private var onboardingIlluminatedPhotoDemo: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 8) {
	            ...
	            Text("It gets decorated right here on your phone and placed in the PDF of your story; and nothing gets uploaded anywhere.")
	                .font(.system(.footnote, design: .serif))
	                .foregroundStyle(BookPalette.ink.opacity(0.58))
	                .lineSpacing(2)
	                .fixedSize(horizontal: false, vertical: true)
    ...
    private var onboardingWickerChoice: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 8) {
	            ...
	            // "You can't lose this" and "he walks off with something of
	            // yours" were withdrawing each other one clause apart. Both
	            // facts are true; only the reassurance had to go.
	            Text("One answer, one roll of the dice. Pick the one you mean. The scene ends either way; the roll only decides whether Wicker walks off with something of yours.")
	                .font(.system(.callout, design: .serif))
	                .foregroundStyle(BookPalette.ink.opacity(0.64))
                .fixedSize(horizontal: false, vertical: true)
    ...
    private var onboardingLivingInkRutProof: some View {
        VStack(alignment: .leading, spacing: 11) {
            HStack {
                // The label must not name the bite before it happens. Calling
                // the card bitten up front tells the reader to watch for a
                // change, which turns a demonstration of inattention into a
                // spot-the-difference they were always going to win — the same
                // mistake the gold highlight made below.
                Label(didLivingInkChange ? "THE BITTEN PAGE" : "TODAY'S PAGE", systemImage: "doc.text")
                    .font(.caption.weight(.black))
                    .tracking(0.8)
                ...
                Image(systemName: "sparkles")
                    .font(.caption.weight(.black))
                    .foregroundStyle(BookPalette.lampGold.opacity(didLivingInkChange ? 0.24 : 0.92))
                    .accessibilityHidden(true)
            }

            Group {
                if didLivingInkChange {
                    Text(livingInkProofParts.before)
                    + Text("_____")
                        .foregroundColor(.gray.opacity(0.66))
                    + Text(livingInkProofParts.after)
                } else {
                    // Set exactly like everything around it. Marking the word
                    // in gold told the reader where to look, which turned a
                    // demonstration of inattention into a spot-the-difference
                    // they were always going to win.
                    Text(livingInkProofParts.before)
                    + Text(livingInkProofParts.word)
                    + Text(livingInkProofParts.after)
                }
            }
            ...
            .lineSpacing(3)
            .fixedSize(horizontal: false, vertical: true)
            .animation(reduceMotion ? .none : .easeOut(duration: 0.24), value: didLivingInkChange)
        }
        .padding(14)
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(BookPalette.page.opacity(0.70), in: RoundedRectangle(cornerRadius: 9, style: .continuous))
        .overlay(alignment: .leading) {
            Rectangle()
                .fill(didLivingInkChange ? Color.gray.opacity(0.68) : BookPalette.lampGold.opacity(0.88))
                .frame(width: 4)
        }
        .clipShape(RoundedRectangle(cornerRadius: 9, style: .continuous))
        .accessibilityElement(children: .combine)
        .accessibilityLabel(
            didLivingInkChange
                ? "The bitten Page now says: \(livingInkProofParts.before), blank, \(livingInkProofParts.after)"
                : "The bitten Page says: \(livingInkProofLine)"
        )
    }

    private func scheduleLivingInkRutProofIfNeeded() {
        livingInkChangeTask?.cancel()
        livingInkChangeTask = nil
        guard step == 0, openingMiniStep == 1 else { return }
        guard pageChangeNoticed.isEmpty else {
            didLivingInkChange = true
            return
        }

        didLivingInkChange = false
        livingInkChangeTask = Task { @MainActor in
            do {
    ...
    private var onboardingRoutineRevealAction: some View {
        Button {
            isOnboardingFieldFocused = false
            guard !pageChangeNoticed.isEmpty, !didRevealRoutineEvidence else { return }
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text(didRevealRoutineEvidence ? "The receipt." : "Touch the page again.")
                        .font(.subheadline.weight(.black))
                        .foregroundStyle(BookPalette.ink.opacity(0.82))
                    Text(didRevealRoutineEvidence ? "Read it." : "I kept a receipt. Look at it.")
                        .font(.system(.callout, design: .serif))
                        .foregroundStyle(BookPalette.ink.opacity(0.64))
                        .fixedSize(horizontal: false, vertical: true)
                }
        ...
        .buttonStyle(.plain)
        .accessibilityLabel(didRevealRoutineEvidence ? "The receipt" : "Touch the page again to see the receipt the Book kept")
    }

    /// Minutes of today already spent at the moment the reader arrived.
    ///
    /// The Book took the *name of the day* off the opening Page, so the only
    /// honest receipt it can hand back is the one already on the clock. No
    /// estimate, no claim about the reader's history: a number they can check
    /// against their own watch.
    private var openingSpentMinutesToday: Int {
        let parts = Calendar.current.dateComponents([.hour, .minute], from: arrivalMoment)
        return (parts.hour ?? 0) * 60 + (parts.minute ?? 0)
    }

    /// Pays out the claim the reveal button makes. This used to promise "a
    /// number" and "a receipt" and then render neither, on the one screen whose
    /// whole job is proving the Book's claims can be checked.
    private var onboardingRoutineReceiptCard: some View {
        VStack(alignment: .leading, spacing: 9) {
            Text("\(openingSpentMinutesToday) minutes of today are already spent.")
                .font(.system(.title3, design: .serif).weight(.black))
                .foregroundStyle(BookPalette.ink)
                .fixedSize(horizontal: false, vertical: true)

            Text("\"I took one word off your page and you didn't feel it go. That's the size of your not-noticing. Now hold that steady across \(openingSpentMinutesToday) minutes.\"")
                .font(.system(.callout, design: .serif))
                .foregroundStyle(BookPalette.ink.opacity(0.78))
                .lineSpacing(2)
                .fixedSize(horizontal: false, vertical: true)
        ...
        .padding(13)
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(BookPalette.page.opacity(0.66), in: RoundedRectangle(cornerRadius: 9, style: .continuous))
        .overlay(alignment: .leading) {
            Rectangle()
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text(didSteadyFirstPage
                         ...
                         : "You came through a Page. Getting up is harder than it sounds.")
                        .font(.system(.callout, design: .serif))
                        .foregroundStyle(BookPalette.ink.opacity(0.64))
                        .fixedSize(horizontal: false, vertical: true)
                }
    ...
    private var onboardingCastSnackArgument: some View {
        ...
        return VStack(alignment: .leading, spacing: 14) {
            ...
            onboardingProse("""
            Zara doesn't look up. "They'll do this about everything. You get used to it. I never have."
            """)
        }
    }
    ...
    private var onboardingPracticeBones: some View {
        onboardingTitle("These Are the Inkbones")
        // Attribution rule for everything inside the Book: an unattributed
        // quote belongs to whoever is on screen, so Zara's lines say so and
        // the Book speaks only when it visibly interrupts. Floating dialogue
        // made "I want to watch how you do it" read as either of them.
        onboardingProse("""
        Zara shakes them out of a pouch. "They settle arguments in here. Anything that could go two ways."
    ...
    private var practiceBonesVerdict: String {
        ...
        case 10...11: return "\(practiceBonesRoll). Wasted on a practice throw. The bones won't do that again for a while."
        case 2...9: return "\(practiceBonesRoll). Perfectly ordinary. It cost you nothing."
        default: return "One. The worst it does, spent on nothing at all. You've got that out of the way. I'd call it thrifty."
        }
    }
    ...
    private var onboardingBeliefEconomyCard: some View {
        VStack(alignment: .leading, spacing: 11) {
            ...
            // No nouns the reader has not met yet. This card used to name the
            // Glow, Cast Members, Compass Runs and quests nine minutes in, to
            // somebody who had encountered one character — a systems help panel
            // wearing Zara's voice. Each of those explains itself later by
            // existing.
            onboardingBeliefRule(
                "YOU EARN IT",
                "By keeping real things, and answering honestly.",
                symbol: "eye"
            )
            onboardingBeliefRule(
                "YOU SPEND IT",
                "On anything in here you want to matter more.",
                symbol: "arrow.up.right.circle"
            )
            onboardingBeliefRule(
                "IT COMES BACK",
                "What you back turns up again, in your Pages and in the story.",
                symbol: "sparkle"
            )
        }
        .padding(13)
    ...
    private var onboardingChapterAffinityPicker: some View {
        // The prose above this picker already says these are five
        // arguments and already says to tap one. These two headers said
        // both again, which meant the reader was told to tap four separate
        // times on one screen.
        VStack(alignment: .leading, spacing: 12) {
            VStack(spacing: 8) {
    ...
    private func archiveInscriptionCoverPhoto(_ image: UIImage) -> URL? {
        ...
        do {
            ...
            let directory = base
                .appendingPathComponent(bundle, isDirectory: true)
                .appendingPathComponent("FirstDoorEditions", isDirectory: true)
                .appendingPathComponent("CoverPhotographs", isDirectory: true)
            try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
            let url = directory.appendingPathComponent("First-Door-Reader-Photograph.jpg")
            try data.write(to: url, options: [.atomic])
            return url
        } catch {
    ...
    private var whisperCueReflection: String {
        ...
        case "both":
            return "Morning knocks. Evening brings the evidence back. Change either bell later; they aren't in charge."
        default:
    ...
    /// The three answers the role is read from. Chip ids, not prose, so the
    /// reader-facing copy can change without renaming anybody.
    private var readerRoleAxes: RoleAxes {
        RoleAxes(
            rut: rutStrongest.nonEmpty,
            alive: mostAlive.nonEmpty,
            magic: magicSource.nonEmpty,
            hands: roleHands.nonEmpty,
            // Everything below happened while the Book was watching. Wicker is
            // deliberately absent: he arrives after the naming, so his Mark
            // stays available for the reader to earn later.
            recallKept: recallAnswered ? recallRemembered : nil,
            caughtTheWord: pageChangeNoticed.nonEmpty.map { $0 == "caught" },
            keptFirstPage: rehearsalChoice.map { $0 == .keep },
            backedTheirBelief: belief.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
                ? nil
                : investedBelief
        )
    }

    /// What the Book calls the reader. A role they swapped to wins over a fresh
    /// reading, so the naming does not quietly undo itself on redraw.
    private var namedRole: ComposedRole {
        if let chosen = ReaderRoleRegistry.role(named: chosenRoleID) {
            return ReaderRoleRegistry.compose(role: chosen, axes: readerRoleAxes)
        }
        return ReaderRoleRegistry.compose(axes: readerRoleAxes)
    }

    /// One swap, offered once. Cycling through twelve would turn a reading into
    /// a menu, and a role you shopped for is not a role that read you.
    private var mayRefuseRole: Bool { refusedRoleID.isEmpty }

    private func refuseNamedRole() {
        let ranking = ReaderRoleRegistry.ranked(axes: readerRoleAxes)
        let current = namedRole.role
        guard let index = ranking.firstIndex(where: { $0.id == current.id }) else { return }
        let next = ranking[(index + 1) % ranking.count]
        BookFeedback.play(.select)
        withAnimation(.spring(response: 0.42, dampingFraction: 0.8)) {
            refusedRoleID = current.id
            chosenRoleID = next.id
        }
    }

    /// The Book shows its working. Every line here is something the reader
    /// typed or tapped in the last three steps: the specificity has to be
    /// earned or the whole beat reads as a horoscope.
    /// What the Book is standing on. The measured lines come first, because
    /// they are the ones the reader cannot argue with: they watched themselves
    /// do these. A dossier resting only on chips invites an audit it fails.
    private var roleReceiptLines: [(String, String)] {
        var lines: [(String, String)] = []
        if recallAnswered {
            let scenario = RoutineRecallProbe.scenario(id: recallScenario)?.noun ?? "an ordinary hour"
            lines.append((
                recallRemembered >= 3 ? "eye.fill" : "cloud.fog",
                recallRemembered >= 3
                    ? "You kept all three small things from the \(scenario). Almost nobody does."
                    : "\(RoutineRecallProbe.askedCount - recallRemembered) of three small things were gone from the \(scenario)."
            ))
        }
        if pageChangeNoticed == "caught" {
            lines.append(("bolt.fill", "You caught the word I took while you were holding the Page."))
        }
        if rehearsalChoice == .keep {
            lines.append(("bookmark.fill", "You kept your first Page rather than letting it wait."))
        }
        if !mostAlive.isEmpty {
            lines.append(("waveform.path.ecg", "You are most alive \(mostAliveTitle.lowercased())."))
        }
        if let detail = awakeMemory.trimmingCharacters(in: .whitespacesAndNewlines).nonEmpty {
            lines.append(("quote.opening", "\u{201C}\(detail)\u{201D}: you fetched that from memory, intact."))
        }
        if !magicSource.isEmpty {
            lines.append(("sparkles", "\(magicSourceTitle) puts a charge through you."))
        }
        if let hands = namedRole.hands {
            lines.append(("hand.raised", hands.gloss))
        }
        if let mark = namedRole.mark {
            lines.append(("seal.fill", "\(mark.name.uppercased()): \(mark.evidence)"))
        }
        return lines
    }
    ...
    private var rutReflection: String {
        switch rutStrongest {
        case "work":
            return "“Then I won't ask work to become magical. I'll look for the moments it failed to swallow.”"
        case "phone":
    ...
    private var magicSourceReflection: String {
        if magicSource == "unsure" {
            return "“Good answer. I won't make something up. I'll show you small things and you can decide what counts.”"
        }
        return "“\(magicSourceTitle). That's one door. I'll start there, and keep enough room to surprise you.”"
    }

    private var momentFateReflection: String {
        switch momentFate {
        case "keep":
            return "“Then I should do more than store it. I should let it return changed, when it can mean more.”"
        case "forget":
            return "“Then I can give it a place before the rest of the day carries it off.”"
        case "blur":
            return "“Then my first job is to interrupt the blur. One small thing worth noticing.”"
        default:
    ...
    private var hiddenMagicReflection: String {
        switch hiddenMagicStance {
        case "yes":
            // Answers the answer. This used to reply to a sceptic ("Fine. I
            // won't fake it") when the reader had just said yes.
            return "“Then you've still got it. Bring me the next one and I'll bite down.”"
        case "maybe":
    ...
    private var inscriptionFallThroughProse: String {
        ...
        Then stone catches your knees beneath a library sky. The Page is back in your hand. A girl with quick grey eyes reaches down.
        """
    }
    ...
    private var inscriptionWickerIntroduction: String {
        ...
        let pagewrightLine: String
        if didDelegateOnboardingPagewright {
            pagewrightLine = "His thumb catches on the Pagewright's crooked edge. “You let a tiny criminal edit your evidence?” The Book bites his cuff before he can decide whether that was praise."
        } else if onboardingPagewrightIsReady {
            pagewrightLine = "His thumb catches on the edge you arranged at the Pagewright's table. “Oh, you made it yourself. That makes this much easier to steal.” The Book bites his cuff."
        } else {
            pagewrightLine = "The Book holds one blank corner away from his fingers."
        }
        let edgeLine: String
        switch comfortBoundary {
        case "gentle":
        ...
        return """
        A boy in a green-black coat steps out from behind a shelf at the worst possible moment, and the Page is out of your hand before you register him moving. He turns it over, reading, walking backwards.
    ...
    /// The naming. Everything before this step was the Book gathering; this is
    /// the first time it says something back about the reader and means it.
    /// The dossier is flattery on purpose, but flattery the reader can check,
    /// because the receipts underneath it are their own words.
    @ViewBuilder
    private var onboardingPagewrightInterlude: some View {
        guidePortrait(mood: "Zara looks up as a pair of scissors comes running along the shelf by itself.")
        onboardingTitle("The Book Refuses Another Question")
        onboardingProse("""
        The answer about your hands is still wet when the Book snaps shut on the next blank card.

        \u{201C}No. Too many questions. Answers are loose things. Make me a Page.\u{201D}

        A small person with ink on both elbows crawls out from behind the spine. Zara catches the scissors before they bite the ribbon.

        \u{201C}Pagewright,\u{201D} she says. \u{201C}Two scraps from the door. Nothing you haven't already given us.\u{201D}

        The Pagewright peers at you. \u{201C}And a photograph from your world, if you want one in the evidence. Then I can make the loose things hold still.\u{201D}
        """)

        if onboardingPagewrightIsReady {
            onboardingPagewrightPreview
            onboardingProse("""
            The Pagewright presses both palms to the finished leaf. The Book shivers from cover to cover.

            \u{201C}Mine,\u{201D} it says.

            \u{201C}Yours,\u{201D} says the Pagewright. \u{201C}That's the arrangement.\u{201D}
            """)
            secondaryOnboardingButton("Move the scraps again", systemImage: "scissors") {
                onboardingPagewrightError = ""
                isOnboardingPagewrightPresented = true
            }
        } else {
            Button {
                onboardingPagewrightError = ""
                ...
                Label("Make the first bound Page", systemImage: "scissors")
                    .font(.subheadline.weight(.black))
                    .frame(maxWidth: .infinity)
                    .padding(.vertical, 12)
            }
            ...
            secondaryOnboardingButton("Let the Pagewright get away with it", systemImage: "wand.and.stars") {
                Task { await letPagewrightGetAwayWithIt() }
            }

            onboardingProse("""
            The Pagewright is already standing on the paste brush. One tap lets them arrange the scraps badly on purpose. You can move everything afterward if their crimes offend you.
            """)
        }

        if !onboardingPagewrightError.isEmpty {
            onboardingPreviewCard(
                symbol: "exclamationmark.triangle",
                title: "The paste wouldn't take.",
                body: onboardingPagewrightError
            )
        }

        continueButton("Let the Book read what I made", disabled: !onboardingPagewrightIsReady)
    }

    @ViewBuilder
    private var onboardingPagewrightPreview: some View {
        #if canImport(UIKit)
        if let image = UIImage(contentsOfFile: onboardingPagewrightImagePath) {
            ...
            .padding(12)
            .background(BookPalette.page.opacity(0.72), in: RoundedRectangle(cornerRadius: 10, style: .continuous))
            .overlay {
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .stroke(BookPalette.lampGold.opacity(0.38), lineWidth: 1)
            }
    ...
    private var onboardingPagewrightIsReady: Bool {
        let path = onboardingPagewrightImagePath.trimmingCharacters(in: .whitespacesAndNewlines)
        return !path.isEmpty
            && onboardingPagewrightSourceSignature == currentOnboardingPagewrightSourceSignature
            && FileManager.default.fileExists(atPath: path)
    }

    private var didDelegateOnboardingPagewright: Bool {
        onboardingPagewrightTitle == "Evidence We Smuggled Through"
    }

    private var currentOnboardingPagewrightSourceSignature: String {
        [
            firstPressTrimmedText,
            firstSouvenir,
            name,
    ...
    private var onboardingPagewrightScraps: [BookPage] {
        let now = arrivalMoment
        let handsTitle = handsChoices.first { $0.id == roleHands }?.title ?? roleHands
        return [
            BookPage(
                id: "first-door-pagewright-true-thing",
                type: .souvenir,
                createdAt: now,
                promptText: "The first true thing",
                userInput: firstPressTrimmedText.nonEmpty ?? (firstSouvenir.nonEmpty ?? openingArrivalExcerpt),
                tags: ["onboarding", "pagewright-source", "first-door"],
                sourceID: "first-door"
            ),
            BookPage(
                id: "first-door-pagewright-belief",
                type: .aboutYou,
                createdAt: now.addingTimeInterval(1),
                promptText: "What the Book learned to call you by",
                userInput: "\(name.nonEmpty ?? "Reader") brought \(snack.nonEmpty ?? "a small ration") and said: \(belief.nonEmpty ?? "I'm still deciding what I believe.")",
                tags: ["onboarding", "pagewright-source", "belief"],
                sourceID: "first-door"
            ),
            BookPage(
                id: "first-door-pagewright-magic",
                type: .souvenir,
                createdAt: now.addingTimeInterval(2),
                promptText: "Where the magic gets in",
                userInput: "Most alive: \(mostAliveTitle). Magic: \(magicSourceTitle). Hands: \(handsTitle).",
                tags: ["onboarding", "pagewright-source", "magic"],
                sourceID: "first-door"
            )
        ]
    }

    private var onboardingPagewrightAchievementContext: BookwideMarginaliaAchievement.Context {
        BookwideMarginaliaAchievement.Context(
            pages: onboardingPagewrightScraps,
            anchors: [],
            entityBeliefOffsets: [:],
            completedBookJumps: 0,
            completedCompassRuns: 0,
            electives: [],
            longGameEvidence: [],
            hasChosenQuill: false
        )
    }

    /// The first Pagewright encounter should feel like creative contraband,
    /// not a mandatory design assignment. The reader may still open the small
    /// worktable and arrange every scrap, but this path lets the Pagewright do
    /// one authored piece of mischief with the exact same reader material.
    /// A photograph the reader has already given this session, used only when
    /// the library declines. Normally nil at the Pagewright's interlude,
    /// because the illuminated-photo beat has not happened yet.
    private var onboardingPagewrightFallbackPhoto: PagewrightPersonalPhoto? {
        #if canImport(UIKit)
        guard let image = onboardingPhotoImage,
              let data = image.jpegData(compressionQuality: 0.88) else { return nil }
        return PagewrightPersonalPhoto(
            data: data,
            aspectRatio: image.size.height > 0 ? image.size.width / image.size.height : 1
        )
        #else
        return nil
        #endif
    ...
    private func letPagewrightGetAwayWithIt() async {
        let template = PagewrightTemplate.softChaos

        // Both first-door paths open on the same spread: two kept scraps and
        // one whole photograph from the reader's own library. This path used
        // to look for a photo in the illuminated-photo flow, which has not run
        // yet, so it always produced three scraps and no photograph while the
        // worktable produced two and a photo.
        let photo = await PagewrightLibraryPhoto.random() ?? onboardingPagewrightFallbackPhoto
        let scraps = onboardingPagewrightScraps
        let pages = photo == nil ? scraps : Array(scraps.dropLast())
        let pullQuotes = Dictionary(
            uniqueKeysWithValues: pages.map { ($0.id, PagewrightText.pullQuote(for: $0)) }
        )
        let pinnedNote = PagewrightPinnedNote(
            text: "The scraps voted against neatness.",
            style: .sticky
        )
        var elements = pages.enumerated().map { index, page in
        ...
        let notePlacement = template.placement(for: 0, kind: .note)
        elements.append(
            PagewrightCanvasElement(
                kind: .note,
                sourceID: pinnedNote.id,
                x: notePlacement.x,
                y: notePlacement.y,
                width: notePlacement.width,
                rotation: notePlacement.rotation,
                z: elements.count
            )
        )

        var photos: [PagewrightPersonalPhoto] = []
        if let photo {
            photos.append(photo)
            elements.append(
                PagewrightCanvasElement(
                    ...
                    x: 0.54,
                    y: 0.52,
                    width: min(0.48, max(0.22, photo.aspectRatio * 0.42)),
                    rotation: -5,
                    z: elements.count
                )
            )
        ...
        let draft = PagewrightDraft(
            title: "Evidence We Smuggled Through",
            note: "The loose answers refused to remain an intake form.",
        ...
        let renderedURL = renderOnboardingPagewright(draft)
        keepOnboardingPagewright(draft, renderedURL: renderedURL)
    }

    private func renderOnboardingPagewright(_ draft: PagewrightDraft) -> URL? {
        #if canImport(UIKit)
        do {
            let url = try onboardingPagewrightArchiveURL()
            try PagewrightPDFWriter.writePNG(draft: draft, to: url)
            onboardingPagewrightError = ""
            return url
        } catch {
            onboardingPagewrightError = "The Pagewright couldn't press the image this time. Nothing you arranged was sent anywhere."
            BookFeedback.play(.error)
            return nil
        }
        #else
    ...
    private func keepOnboardingPagewright(_ draft: PagewrightDraft, renderedURL: URL?) {
        let url = renderedURL ?? renderOnboardingPagewright(draft)
        guard let url, FileManager.default.fileExists(atPath: url.path) else { return }
        onboardingPagewrightTitle = draft.title
        onboardingPagewrightTemplate = draft.template.rawValue
        onboardingPagewrightImagePath = url.path
        onboardingPagewrightSourceSignature = currentOnboardingPagewrightSourceSignature
        onboardingPagewrightError = ""
        BookFeedback.play(.braidComplete)
    }

    private func onboardingPagewrightArchiveURL() throws -> URL {
        let base = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
        let bundle = Bundle.main.bundleIdentifier ?? "com.openclaw.enchantify.insidecover"
        let directory = base
            .appendingPathComponent(bundle, isDirectory: true)
            .appendingPathComponent("FirstDoorEditions", isDirectory: true)
            .appendingPathComponent("Pagewright", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        return directory.appendingPathComponent("First-Door-Pagewright.png")
    }

    @ViewBuilder
    private var onboardingRoleReveal: some View {
        let composed = namedRole
        let role = composed.role

        guidePortrait(mood: "Zara has stopped writing. She is reading back what she wrote.")
        onboardingProse("""
        "Right," Zara says, and turns the Page around so you can see it.

        "It's already decided what you are. It watched what you did more than what you said. Whether you caught the word it took. Whether you kept the first Page or let it wait. Whether you put anything behind what you believe."

        "Usually it waits a month."
        """)

        VStack(alignment: .leading, spacing: 14) {
            Text("THE BOOK NAMES YOU")
                .font(.system(size: 10, weight: .black))
                .tracking(1.4)
                .foregroundStyle(BookPalette.lampGold)

            Text(composed.fullName)
                .font(.system(size: 34, weight: .bold, design: .serif))
                .foregroundStyle(BookPalette.ink)
                .fixedSize(horizontal: false, vertical: true)
                .shadow(color: BookPalette.lampGold.opacity(0.24), radius: 10, x: 0, y: 3)

            if let hands = composed.hands {
                Text(hands.name)
                    .font(.system(size: 13, weight: .black))
                    .tracking(0.6)
                    .foregroundStyle(BookPalette.teal)
        ...
        .padding(18)
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(BookPalette.page.opacity(0.72), in: RoundedRectangle(cornerRadius: 12, style: .continuous))
        .overlay {
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .stroke(BookPalette.lampGold.opacity(0.42), lineWidth: 1.5)
        }
        .id("onboarding-role-\(role.id)")
        .transition(.opacity.combined(with: .scale(scale: 0.97)))
        .modifier(OnboardingSectionArrivalModifier(kind: .artifact, delay: 0.06))

        if let epithet = composed.epithet {
            onboardingPreviewCard(
                symbol: "cloud.fog",
                title: "And I know where the grey gets in",
                body: epithet.cost
            )
        }
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            ForEach(roleReceiptLines.indices, id: \.self) { index in
                let line = roleReceiptLines[index]
        ...
        .padding(14)
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(BookPalette.paper.opacity(0.52), in: RoundedRectangle(cornerRadius: 10, style: .continuous))
        .modifier(OnboardingSectionArrivalModifier(kind: .section, delay: 0.14))

        onboardingPreviewCard(
            symbol: "person.crop.circle.badge.checkmark",
            title: "\(role.patronName) has been told",
            body: "Every \(role.bareName) in here is somebody's business. \(role.patronName) has taken an interest in yours."
        )
        ...
        if mayRefuseRole {
            ...
            Text("You turned the first name down. I'm keeping that one too.")
                .font(.footnote)
                .foregroundStyle(BookPalette.ink.opacity(0.46))
                .frame(maxWidth: .infinity, alignment: .center)
        }

        continueButton("Take the name")
    }
    ...
    /// The Curse, evidenced: three real studies and one extrapolation the Book
    /// owns out loud.
    ///
    /// The extrapolation used to be hedged three separate times — "here is my
    /// extrapolation", "I am naming the consequence", "I don't intend to call
    /// that harmless" — and a claim disclaimed three times reads as a Book that
    /// does not believe itself. The line between what the researchers found and
    /// what the Book infers is now drawn exactly once, in its own voice, and
    /// the inference then stands at full strength.
    private var onboardingNotYourFaultCard: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            // First proof: a thing the size of a gorilla can go past unseen.
            VStack(alignment: .leading, spacing: 9) {
                Text("THE GORILLA NOBODY SAW")
                    .font(.system(.title3, design: .serif).weight(.black))
                    .foregroundStyle(BookPalette.ink)

                Text("In a 1999 study, people watched two teams pass basketballs and counted the passes made by one team. Then someone in a gorilla suit walked straight through the game.")
                    .font(.system(.callout, design: .serif))
                    .foregroundStyle(BookPalette.ink.opacity(0.72))
                    .lineSpacing(2)
                    .fixedSize(horizontal: false, vertical: true)
                ...
                HStack(alignment: .firstTextBaseline, spacing: 8) {
                    Text("56%")
                        ...
                        .foregroundStyle(BookPalette.gold)
                    Text("DIDN'T REPORT SEEING IT")
                        .font(.system(size: 11, weight: .black))
                        .tracking(0.8)
                        .foregroundStyle(BookPalette.ink.opacity(0.64))
                }
                .accessibilityElement(children: .combine)
                .accessibilityLabel("Fifty-six percent did not report seeing the person in the gorilla suit.")

                Label("Simons & Chabris, Perception (1999)", systemImage: "text.book.closed")
                    .font(.footnote.weight(.bold))
                    .foregroundStyle(BookPalette.ink.opacity(0.52))

                Text("\"A whole gorilla. Right in front of them. They were busy counting. That's the size of thing you can miss.\"")
                    .font(.system(.title3, design: .serif).weight(.bold))
                    .foregroundStyle(BookPalette.teal.opacity(0.94))
                    .lineSpacing(2)
                    .fixedSize(horizontal: false, vertical: true)
            ...
            // Second proof: it happens about half of the time.
            VStack(alignment: .leading, spacing: 8) {
                ...
                // The citation belongs against the sentence it backs, not
                // stranded at the foot of the card where it reads as a credit
                // rather than as evidence for that specific claim.
                Label("Killingsworth & Gilbert, Science (2010)", systemImage: "text.book.closed")
                    .font(.footnote.weight(.bold))
                    .foregroundStyle(BookPalette.ink.opacity(0.52))

                Text("\"They called it mind-wandering. I call it autopilot: hands moving, road passing, water boiling, attention somewhere else.\"")
                    .font(.system(.title3, design: .serif).weight(.bold))
                    .foregroundStyle(BookPalette.ink)
                    .lineSpacing(2)
                    .fixedSize(horizontal: false, vertical: true)
            ...
            // The one place the Book leaves the findings behind. Marked once,
            // then said at full strength.
            VStack(alignment: .leading, spacing: 10) {
                Text("\"That's their number. The next part is mine.\"")
                    .font(.system(.callout, design: .serif).weight(.black))
                    .foregroundStyle(BookPalette.ink.opacity(0.80))
                    .fixedSize(horizontal: false, vertical: true)
                ...
                HStack(alignment: .center, spacing: 10) {
                    ...
                    VStack(alignment: .leading, spacing: 0) {
                        Text("4.69")
                            .font(.system(size: 34, weight: .black, design: .rounded))
                            .foregroundStyle(BookPalette.teal)
                        Text("YEARS' WAKING HOURS ELSEWHERE")
                            .font(.system(size: 9, weight: .black))
                            .tracking(0.8)
                            .foregroundStyle(BookPalette.ink.opacity(0.52))
                    }
                ...
                .padding(.vertical, 2)
                .accessibilityElement(children: .combine)
                .accessibilityLabel("If the study's average held across ten years of waking hours, 4.69 years' worth of those hours would pass with the mind elsewhere.")

                Text("\"Hold 46.9% across ten years and four years and eight months of your waking life go by with you somewhere else. Not asleep. Not resting. Gone.\"")
                    .font(.system(.title3, design: .serif).weight(.black))
                    .foregroundStyle(BookPalette.teal.opacity(0.94))
                    .lineSpacing(2)
                    .fixedSize(horizontal: false, vertical: true)
            ...
            // Why the wandered hours do not stay. The useful-wandering clause
            // keeps the claim accurate without softening it.
            VStack(alignment: .leading, spacing: 6) {
                Text("Attention is how a moment gets into memory. When attention leaves, less of what's in front of you gets in. Some wandering is useful: plans, daydreams. It still doesn't save the room you were standing in.")
                    .font(.system(.callout, design: .serif).weight(.semibold))
                    .foregroundStyle(BookPalette.ink.opacity(0.78))
                    .lineSpacing(2)
                    .fixedSize(horizontal: false, vertical: true)

                Label("Blondé et al., Neuroscience & Biobehavioral Reviews (2022)", systemImage: "text.book.closed")
                    .font(.footnote.weight(.bold))
                    .foregroundStyle(BookPalette.ink.opacity(0.52))
            }

            // The reader's own two answers, which outrank every study above.
            Text(recallContrastLine)
                .font(.system(.callout, design: .serif).weight(.bold))
                .foregroundStyle(BookPalette.ink.opacity(0.84))
                .lineSpacing(2)
                .fixedSize(horizontal: false, vertical: true)
            ...
            Text(
                recallRemembered >= RoutineRecallProbe.askedCount
                    ? "\"That's the Curse. Not a percentage, and not a verdict on you. I'm not a cure. I keep watch on what you already notice.\""
                    : "\"That's the Curse. The part of your life that happens and never becomes anywhere you can go back to. I'm not a cure. I keep the pieces it misses.\""
            )
                .font(.system(.title3, design: .serif).weight(.black))
                .foregroundStyle(BookPalette.ink)
                .lineSpacing(2)
    ...
    private func resetOnboardingInputs() {
        ...
        photoGildSweep = 0
        isOnboardingPagewrightPresented = false
        onboardingPagewrightImagePath = ""
        onboardingPagewrightTitle = ""
        onboardingPagewrightTemplate = ""
    ...
    private func advanceOpeningMiniStep() {
        isOnboardingFieldFocused = false
        guard openingMiniStep < Self.openingMiniStepCount - 1 else { return }
        BookFeedback.play(.openPage)
    ...
    private func onboardingResult(useDefaults: Bool) -> Result {
        ...
        let trimmedAwakeMemory = awakeMemory.trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        return Result(
            ...
            mostAlive: mostAlive,
            awakeMemory: trimmedAwakeMemory,
            magicSource: magicSource,
            roleHands: roleHands,
            roleID: useDefaults ? "" : namedRole.role.id,
            refusedRoleID: useDefaults ? "" : refusedRoleID,
            snack: trimmedSnack,
